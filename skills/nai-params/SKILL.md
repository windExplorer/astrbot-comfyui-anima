...
## 配合 nai-codex 法典（提示词词库）拼 NAI tag

当你要给 NAI 画面注入【具体画风 / 画师串 / 角色 / 服装 / 体位 / 异种 / 捆绑】等需要精确 tag 的效果时，**不要自己凭记忆拼 NAI 标签**（容易缺画师串 / 权重记号、画错味）——调用插件提供的 `nai_codex` 工具检索「所长 NovelAI 法典」，拿到现成 tag 串后原样拼进 `comfyui_draw` 的 `prompt` 即可。

- `nai_codex` 的 `scope` 默认 `sfw`（常规册）；仅在用户明确要涩涩内容时才传 `nsfw-a` / `nsfw-b`。
- 词库内容、章节导航与 tag 语法见技能 `skills/nai-codex/SKILL.md`。
