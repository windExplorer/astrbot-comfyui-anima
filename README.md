...
- **统计**（v3.4.0 新增）：按用户统计生图数量排行，支持「今天 / 近 3 天 / 近 7 天 / 全部」四个范围，排行展示用户与 QQ 两列，并带「合并插件记录」开关（v3.4.3 新增：开启后把 PrivateCompanion 等插件来源记录合并为一条）；另有近一天的生图数量面积图（24 小时滚动窗口，按小时分桶，X 轴只显示「小时:00」，数据点悬浮/直接显示数量）。两个面板都带「刷新」按钮，点击立即重新拉取数据。
