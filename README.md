...
- `draw_auto`：AI 对话自动出图控制（v4.3.3 新增，v4.3.4 增加 default_count）。`default_count`（默认 3）为默认出图张数——AI 对话中用户说「来几张 / 来多张」或完全没提数量时，默认出这么多张；用户明确说具体数（如「来 3 张」）才按具体数出。`max`（默认 3）为单次请求最多出图数（硬上限），无论默认张数还是用户明确说的数字都不会超过它（管理员豁免时可突破）；`window`（默认 90 秒）为连发判定时间窗（此秒数内连续出图视为同一次请求）；`admin_exempt`（默认 true）管理员是否豁免此限制。
