...
把一段对话「存档成剧情」：进入后你和 bot 的聊天、期间生成的图都会被自动归档成一份「剧情档案」，退出时还能让 LLM 自动生成一段摘要。适合跑剧情推演、角色扮演长线故事等场景。

**开启与进入**
1. 插件配置页开启 `story_mode.enabled`，并按需在 `whitelist_users` 填允许进入的用户（留空 = 所有用户均可进入；填写后仅名单内可进，其他人会收到「你暂无剧情模式权限～」）。
2. 在**私聊**里发送进入触发词（默认：`进入剧情模式`、`开启剧情模式`、`开始剧情模式`、`剧情开始`，可在 `trigger_keywords` 修改），bot 回「已进入剧情推演模式…」即开始。
   - 群聊、非白名单用户无法进入；进入词按「整句等于 / 以关键词开头」判定，避免「今天讲个剧情模式的故事」误触发。

**使用**
- 进入后像平常一样聊天即可：你发的每句话、bot 的每句回复都会被记入当前剧情档案。
- 想出图直接用自然语言描述（如「画一张 xxx」），bot 调用 `comfyui_draw` 出图，图片会**自动关联**到这份档案（可在 WebUI 预览），不需要你手动保存。
- 剧情推进风格（步骤规划、每几步配一张图等）由你给 bot 的 **system prompt** 决定——插件只负责开关、白名单与自动记录，不内置推演剧本。建议在 bot 的 system prompt 里写清「进入剧情模式后如何推进场景」。
- 退出时私聊发送退出触发词（默认：`退出剧情模式`、`结束剧情模式`、`退出剧情`、`结束剧情`、`停`，可在 `exit_keywords` 修改），bot 结束并记录。`auto_summary` 开启时，会调用对话模型把这段对话浓缩成一段摘要写进档案；「停」只认整句，避免日常聊天误退。
