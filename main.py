...
_WF_HINTS = {
    ...
    "not_found": [
        "唔…没有「{wf}」这个工作流啦 (´• ω •`)ﾉ 奴家先用默认工作流帮你画咯~",
        "咦，奴家翻遍图纸也没找到「{wf}」呢 (｡•́︿•̀｡) 那…先用默认的凑合画一张好不好嘛~",
        "呜，没有叫「{wf}」的工作流啦，要不奴家用默认的给你画一张？乖，先画咯~ (✿◡‿◡)",
    ],
    # 只写了工作流名，却没给提示词
    "no_arg": [
        "画谁捏~ 给奴家一句提示词嘛 (｡•́︿•̀｡) 例如：/画真人 一个女孩",
        "唔，工作流是「{wf}」没错，可你还没说画啥呀 (´• ω •`) 快给句提示词嘛~",
...
（--名称[:权重] 为 LoRA 简写，如 --安魂曲:1、--安魂曲:0.5，冒号支持 : 与 ：；--名称/预设名 引用该 LoRA 的预设提示词，如 --安魂曲/预设1）"""
    ...
    async def llm_draw(
        ...
        3. 用户吐槽/提醒你没画图（如「你咋忘了怎么画图了」「你又没画」「你怎么不画了」）：
           这本身就是催促画图，必须立即调用本工具，不要只用文字答应「我这就画」。
