...
（--名称[:权重] 为 LoRA 简写，如 --安魂曲:1、--安魂曲:0.5，冒号支持 : 与 ：；--名称/预设名 引用该 LoRA 的预设提示词，如 --安魂曲/预设1）"""
    ...
    async def llm_draw(
        ...
        Args:
            ...
            count(number): 本次要生成的图片张数。三种情况：①用户完全没提数量（如"画张图"）→ 不传 count（默认 1 张）；②用户说具体数（如"来 3 张"）→ 传 N；③用户说"来几张/再来几张/发点图/一些/多画几张"这类泛化多张（没具体数字）→ 传 3。一次不要超过 9 张，受插件单次上限约束（默认最多 3 张）。注意：即使 count 大于 1，也只对「用户当前这条消息的明确要求」生效，不要自己擅自连续多张。
    ...
    async def llm_img2img(
        ...
        Args:
            ...
            count(number): 本次要生成的图片张数。三种情况：①用户完全没提数量 → 不传 count（默认 1 张）；②用户说具体数（如"来 3 张"）→ 传 N；③用户说"来几张/再来几张/发点图/一些/多画几张"这类泛化多张（没具体数字）→ 传 3。一次不要超过 9 张，受插件单次上限约束（默认最多 3 张）。注意：即使 count 大于 1，也只对「用户当前这条消息的明确要求」生效，不要自己擅自连续多张。
