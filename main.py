...
（--名称[:权重] 为 LoRA 简写，如 --安魂曲:1、--安魂曲:0.5，冒号支持 : 与 ：；--名称/预设名 引用该 LoRA 的预设提示词，如 --安魂曲/预设1）"""
    ...
    async def llm_draw(
        ...
        - 用户说了具体数字（如「来 3 张」「来 5 张」）→ 把数字填进 count。
        - 用户说了「来几张 / 再来几张 / 发点图 / 一些 / 多画几张」这类泛化多张表达（没有具体数字）→ 把 count 填 3。
        - 用户要【多个不同画面】（如「分别画猫、狗和兔子」「白天、晚上、深夜」）→ 用 prompts 一次传多条，每项一个画面。
        - ★「各来 X 张」/「每个 X 张」/「每张 X 版」是【乘法】：把每个分组对象写成 prompts 的一项，再把 X 填进 count，
          总张数 = 分组数 × X。例如「白天、晚上、深夜各来两张」→ prompts 传 3 条（白天/晚上/深夜）+ count=2 → 共 6 张。
          【只传 count=2 是错的】——那样只会出同一个画面的 2 张，画面根本不会分组，等于没满足用户的要求。
        ★一条用户请求只调用本工具【一次】：要几张、几个画面，都在这一次调用里用 count / prompts 说完。
          不要把一次请求拆成多次调用——本轮已经出过图之后再调用，会被直接拦回、不会产生任何新图，纯属白费一轮。
