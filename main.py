...
class ComfyUIDrawPlugin(Star):
    def __init__(self, context: Context, config: AstrBotConfig | None = None) -> None:
        ...
        # 底模库（v7.0.0）：底模实体化管理（支持语言/提示词风格/C站链接/封面）
        ...
        try:
            try:
                ...
                from basemodel_store import BaseModelStore
            self.basemodels = BaseModelStore(self.data_dir)
            logger.info(f"【初始化】 底模库已就绪: {self.basemodels.db_path}")
        except Exception as e:
            logger.warning(f"【初始化】 底模库初始化失败（功能不可用）: {e}", exc_info=True)

        # 基础工作流库（v7.0.0）：上传/解析/入库，出图工作流按 base_id 引用
        ...
        try:
            try:
                ...
                from workflow_store import WorkflowStore
            self.workflow_store = WorkflowStore(self.data_dir)
            logger.info(f"【初始化】 基础工作流库已就绪: {self.workflow_store.db_path}")
        except Exception as e:
        ...
        try:
            ...
            for _dep_name in (
                ...
                "comfyui_client", "workflow_builder",
    ...
    _CARD_DEFAULTS = {
        ...
        # v7.3.0：9 套主题（teal/night/indigo/violet/amber/sunset/crimson/rose/graphite），
        # 失败/拦截自动切朱红，不受此值影响。
        "theme": "teal",
    ...
    # ---- 卡片取数（各入口共用；任何一项取不到就留空，渲染端会自动跳过） ----
    ...
    def _card_device(self, srv_key: str = "", platform_name: str = "") -> str:
        ...
        if platform_name:
            return f"云端 {platform_name}"
        try:
            for s in (self._cfg("comfyui_servers", []) or []):
                if self._server_key(s) == srv_key:
                    _dev = str(s.get("device") or "").strip()
                    _nm = str(s.get("name") or "").strip()
                    return _dev or _nm or srv_key
        except Exception:
            pass
        return srv_key or ""

    @staticmethod
    def _card_chips(pairs: list[tuple[str, object]]) -> list[str]:
        """(标签, 值) → 胶囊文案；空值/占位值自动跳过。"""
        out: list[str] = []
        for k, v in pairs:
            if v is None:
                continue
            _s = str(v).strip()
            if not _s or _s in ("-1", "-1.0", "None", "无"):
                continue
            out.append(f"{k} {_s}")
        return out

    def _card_loras_of(self, wf: dict | None) -> list[str]:
        """工作流自带的启用 LoRA → 胶囊文案（失败卡等拿不到运行时 LoRA 时用）。"""
        try:
            out: list[str] = []
            for l in (self._loras_of(wf or {}) or []):
                if not isinstance(l, dict) or l.get("enabled") is False:
                    continue
                _nm = str(l.get("name") or "").strip()
                if not _nm:
                    continue
                _w = l.get("weight")
                out.append(f"{_nm} {_w if _w is not None else '默认'}")
            return out
        except Exception:
            return []

    def _card_would_send(self, event, state: str) -> bool:
        """预计这次会不会发卡（用于决定是否还把报告文字并进配文，避免同一信息出现两次）。"""
        try:
            cfg = self._card_cfg()
    ...
    def _card_fail_info(self, *, wf: dict | None = None, prompt: str = "",
        ...
        return {
            ...
            "params": self._card_chips([
                ("尺寸", size),
                ("模型", model if platform_name else None),
                ("步数", _sd.get("steps")),
                ("CFG", _sd.get("cfg")),
                ("采样器", _sd.get("sampler_name")),
                ("调度器", _sd.get("scheduler")),
                ("种子", seed),
            ] + list(extra or [])),
            "prompt": prompt,
        }

    async def _send_draw_card(self, event, info: dict, state: str) -> bool:
        ...
        try:
            ...
            logger.info(
                f"【出图卡片】 已发送 state={state} "
                f"theme={mod.norm_theme(str(cfg.get('theme') or ''), state)}"
            )
            return True
        except Exception as e:
    ...
    async def _card_or_text(self, event, text: str, *, info: dict | None = None,
        ...
        卡片只是「换个更好看的壳」，信息与原来那条文字完全一致（文案进卡片的原因栏）。
        """
        _info = dict(info or {})
        if not _info.get("reason"):
            _info["reason"] = text
        try:
            if await self._send_draw_card(event, _info, state):
                return
        except Exception as e:
            logger.warning(f"【出图卡片】 失败卡异常（降级为文字）: {e}")
        try:
    ...
    def _record_failed(
        ...
        # 失败计数（卡片右上角「今日已出图」只统计成功，失败单独记，便于排查）
        try:
            self._card_module().bump_today(self.data_dir, ok=False)
        except Exception:
            pass
        try:
        ...
        *,
        ...
        try:
            ...
            await self._send_draw_card(event, {
                "kicker": self._card_kicker(platform_name=pname, model=model),
                ...
                "params": self._card_chips([
                    ("模型", model),
                    ("尺寸", f"{_w}x{_h}"),
                    ("步数", steps if steps is not None else _pdef.get("steps")),
                    ("引导", cfg if cfg is not None else _pdef.get("scale")),
                    ("采样器", sampler or _pdef.get("sampler")),
                    ("噪声调度", noise_schedule or _pdef.get("noise_schedule")),
            ...
            logger.warning(f"【出图卡片】 平台卡构建失败（忽略）: {_ce}")
        ...
        try:
            ...
            await self._card_or_text(
                event, self._friendly_error(e, f"平台「{pname}」生图"),
                info=self._card_fail_info(
                    platform_name=pname, model=model, prompt=positive,
                    size=f"{_w}x{_h}", seed=seed,
        ...
        if not images:
            await self._card_or_text(
                event, self._cute("no_image"),
                info=self._card_fail_info(
                    platform_name=pname, model=model, prompt=positive,
                    size=f"{_w}x{_h}", seed=seed, cost=time.time() - _draw_start,
                ),
            )
            self._record_failed(
                event, positive, {"name": f"[平台 {pname}]"}, False, None,
        ...
        if _ok_sent:
            ...
            try:
                _lv = locals()
                _plat_cost = time.time() - _draw_start
                await self._send_draw_card(event, {
                    "kicker": self._card_kicker(platform_name=pname, model=model),
                    "workflow": f"{pname} · 文生图",
    ...
    def _load_from_base(self, wf: dict) -> tuple[dict, dict]:
        ...
        并把解析注记注水进 wf **副本**（绝不改配置原件）：
        positive/negative 节点+字段、宽高节点、输出节点、LoRA 锚点、
        图生图节点、放大链位置、采样器节点与默认参数。
        返回 (wf副本, prompt)。
        """
        base_id = str(wf.get("base_id") or "").strip()
        store = self.workflow_store
        if store is None:
            raise ValueError("基础工作流库未初始化")
        rec = store.get(int(base_id), with_json=True) if base_id.isdigit() else None
        if not rec:
            raise ValueError(f"基础工作流（base_id={base_id}）不存在或已被删除，请到「基础工作流」页检查")
        if not rec.get("parse_ok"):
            raise ValueError(f"基础工作流「{rec.get('name')}」解析未通过：{rec.get('parse_msg') or '未知原因'}，请重新解析或重新上传")
        prompt = json.loads(rec.get("wf_json") or "{}")
        roles = rec.get("roles") or {}

        wf = {**wf}  # 副本：后续所有注入都写进副本，配置原件只读
        pos = roles.get("positive") or {}
        neg = roles.get("negative") or {}
        lat = roles.get("latent") or {}
        save = roles.get("save") or {}
        up = roles.get("upscale") or {}
        wf["positive_node"] = pos.get("node", "")
        wf["positive_field"] = pos.get("field", "text")
        wf["negative_node"] = neg.get("node", "")
        ...
        # 内置 LoRA：全部节点保护（用户配置的 LoRA 只注入新节点，绝不覆写内置），
        # 禁用走 _apply_base_overrides 的强度置 0
        _bl = roles.get("builtin_loras") or []
        wf["_builtin_loras"] = _bl
        wf["_builtin_lora_nodes"] = [b.get("node") for b in _bl if b.get("node")]
        if up.get("loader"):
            wf["upscale_node_id"] = up["loader"]
        elif "upscale_node_id" in wf:
            wf["upscale_node_id"] = ""
        # 采样器覆盖所需的注记（内部键，带 _ 前缀不入配置）
        wf["_sampler_node"] = roles.get("sampler", "")
        ...
        if str(wf.get("ov_steps") or "").strip():
            try:
                wf["default_steps"] = int(float(wf["ov_steps"]))
                wf["steps_off"] = False
            except (TypeError, ValueError):
                pass
        else:
            wf["steps_off"] = True
        if str(wf.get("ov_cfg") or "").strip():
            try:
                wf["default_cfg"] = float(wf["ov_cfg"])
                wf["cfg_off"] = False
            except (TypeError, ValueError):
                pass
        else:
            wf["cfg_off"] = True
        # denoise：文生图只读（沿用基础图原值）；图生图且填了才注入
        if roles.get("kind") == "img2img" and str(wf.get("ov_denoise") or "").strip():
            try:
                wf["default_denoise"] = float(wf["ov_denoise"])
                wf["denoise_off"] = False
            except (TypeError, ValueError):
                pass
        else:
            wf["denoise_off"] = True
    ...
    def _apply_base_overrides(self, wf: dict, prompt: dict) -> None:
        """基础工作流覆盖：采样器名/调度器、放大三态、清理注入、保存节点替换。"""
        # 内置 LoRA 禁用（强度置 0，节点保留以免连线断裂）
        for _bn in wf.get("_disable_builtin_loras") or []:
            node = workflow_builder._get_node(prompt, _bn)
            if not node:
                continue
            _ct = (node.get("class_type") or "")
            _in = node.setdefault("inputs", {})
            _in["strength_model"] = 0.0
            if "strength_clip" in _in:
                _in["strength_clip"] = 0.0
            logger.info(f"【LoRA】 内置 LoRA 节点 {_bn}（{_ct}）已按配置禁用（强度 0）")

        # 采样器 / 调度器
        _sn = (wf.get("ov_sampler") or "").strip()
        _sd = (wf.get("ov_scheduler") or "").strip()
        if (_sn or _sd) and wf.get("_sampler_node"):
            if workflow_builder.set_sampler_node(prompt, wf["_sampler_node"], _sn, _sd):
                logger.info(f"【采样器覆盖】 sampler={_sn or '沿用'} scheduler={_sd or '沿用'}")

        save_node = wf.get("_save_node") or ""
        ...
        if umode == "bypass" and wf.get("_upscale_apply"):
            ...
            if _um:
                new_apply = workflow_builder.inject_upscale(prompt, save_node, _um)
                if new_apply:
                    wf["_upscale_apply"] = new_apply
                    logger.info(f"【放大】 已注入放大链（模型 {_um}）")
            else:
                logger.warning("【放大】 未填放大模型名，跳过注入")
        elif umode == "inject" and wf.get("_upscale_apply"):
            logger.info("【放大】 基础图已内置放大链，注入跳过")

        # 清理显存：基础图没有且启用 → 注入
        if (wf.get("cleanup_mode") or "").strip().lower() == "inject":
            if not (wf.get("_cleanup_nodes") or []):
                new_cleanup = workflow_builder.inject_cleanup(prompt, save_node)
                if new_cleanup:
                    logger.info(f"【清理显存】 已注入 easy cleanGpuUsed（节点 {new_cleanup}）")

        # 保存节点替换：配置了格式或质量且基础图保存节点不支持对应字段
        _fmt = (wf.get("save_format") or "").strip()
        _q = str(wf.get("save_quality") or "").strip()
        if _fmt or _q:
            save = prompt.get(save_node) or {}
            save_ct = (save.get("class_type") or "").lower()
            need_replace = ("extended" not in save_ct) or (
                _fmt and "output_ext" not in (save.get("inputs") or {})
            )
            if need_replace:
                new_save = workflow_builder.replace_save_node(
                    prompt, save_node, _fmt, _q if _q else None
                )
                if new_save:
                    wf["output_node"] = new_save
                    logger.info(f"【保存】 已替换为 SaveImageExtended（格式 {_fmt or '默认'}，质量 {_q or '默认'}）")
            else:
                _in = save.setdefault("inputs", {})
                if _fmt and "output_ext" in _in:
                    _in["output_ext"] = _fmt
                if _q:
                    try:
                        _in["quality"] = int(_q)
                        ...
                        pass
                logger.info(f"【保存】 已覆盖格式/质量（格式 {_fmt or '沿用'}，质量 {_q or '沿用'}）")
    ...
    async def _do_draw(
        ...
        try:
            ...
            if "已停用" in msg:
                ...
                await self._card_or_text(
                    event, msg,
                    info=self._card_fail_info(wf=wf, prompt=positive,
                                              is_img2img=is_img2img,
                                              cost=time.time() - _draw_start),
                )
            else:
                ...
                await self._card_or_text(
                    event, f"绘图配置有误：{_sanitize_exc_text(str(e), 160)} 请联系管理员调整。",
                    info=self._card_fail_info(wf=wf, prompt=positive,
                                              is_img2img=is_img2img,
                                              cost=time.time() - _draw_start),
                )
        ...
        try:
            if (wf.get("base_id") or "").strip():
                # v7.0.0：新版工作流——从基础工作流库克隆底图，并把解析注记注水进
                # wf 副本（positive_node/field 等），后续管线零改动直接可用。
                wf, prompt = self._load_from_base(wf)
                self._apply_base_overrides(wf, prompt)
            else:
            ...
            await self._card_or_text(
                event, self._friendly_error(e, "工作流加载", "workflow"),
                info=self._card_fail_info(wf=wf, prompt=positive, is_img2img=is_img2img,
                                          srv_key=str(locals().get("srv_key") or ""),
                                          cost=time.time() - _draw_start),
            )
            return
        ...
        enabled = workflow_builder.apply_loras(
            ...
            model_only=True,
            protected_nodes=set(wf.get("_builtin_lora_nodes") or []),
        )
        ...
        # v7.0.0：工作流配置了固定种子（fixed_seed）且用户未显式指定时，用固定值
        _seed_in = seed
        if not _seed_in:
            _fs = str(wf.get("fixed_seed") or "").strip()
            if _fs.isdigit():
                _seed_in = int(_fs)
                logger.info(f"本次种子: 使用工作流固定种子 {_seed_in}")
        seeds_used = workflow_builder.randomize_seed(prompt, _seed_in)
        if seeds_used:
        ...
        try:
            ...
            try:
                ...
                await self._card_or_text(
                    event, self._friendly_error(e, "提交任务"),
                    info=self._card_fail_info(
                        wf=wf, prompt=positive, is_img2img=is_img2img, srv_key=srv_key,
                        size=_size, seed=(seeds_used[0] if seeds_used else None),
                        cost=time.time() - _draw_start,
                    ),
                )
                return
            ...
            if not prompt_id:
                ...
                await self._card_or_text(
                    event, self._cute("no_task_id"),
                    info=self._card_fail_info(
                        wf=wf, prompt=positive, is_img2img=is_img2img, srv_key=srv_key,
                        size=_size, seed=(seeds_used[0] if seeds_used else None),
                        sampler=locals().get("_card_sampler") or {},
                        cost=time.time() - _draw_start,
                    ),
                )
                self._record_failed(
                ...
                for img in images:
                    ...
                    if not _nsfw_blocked and (self._cfg_image_caption().get("enabled", True)):
                        ...
                        if (self._cfg("show_draw_report", False)
                                and self._cfg_image_caption().get("merge_draw_report", True)
                                and not self._card_would_send(event, "done")):
                    ...
                    # 卡片右上角「今日已出图」计数（只算真正发出去的图；被 NSFW 拦截的不算）
                    if not _nsfw_blocked:
                        try:
                            self._card_module().bump_today(self.data_dir, ok=True)
                        except Exception:
...
（--名称[:权重] 为 LoRA 简写，如 --安魂曲:1、--安魂曲:0.5，冒号支持 : 与 ：；--名称/预设名 引用该 LoRA 的预设提示词，如 --安魂曲/预设1）"""
    ...
    async def cmd_character(self, event: AstrMessageEvent):
        ...
        try:
            ...
            if _action in ("锚点", "anchor", "anchors"):
                ...
                return
            if _action in ("参考图", "ref", "refs"):
                await self._character_ref(event, store, _tokens[1:], cfg)
                return
            if _action in ("补全", "suggest", "fetch"):
                await self._character_suggest(event, store, _tokens[1:], cfg)
                return
    ...
    async def _character_ref(self, event, store, tokens: list[str], cfg: dict):
        ...
        if sub in ("删", "删除", "del", "delete"):
            ...
            if len(tokens) < 3 or not tokens[2].strip().isdigit():
                ...
                return
            ref = store.get_ref(int(tokens[2]))
            if ref is None or int(ref.get("character_id") or 0) != int(ch["id"]):
                await self._send(event, f"「{ch['name']}」下没有 id={tokens[2]} 的参考图。")
                return
            store.delete_ref(int(tokens[2]))
            await self._send(event, f"已删除「{ch['name']}」的参考图 id={tokens[2]}。")
            return
        if sub in ("记住", "add", "新增"):
            if not cfg.get("allow_user_edit", True) and not self._is_admin(event):
                await self._send(event, "当前配置仅允许管理员修改角色卡片（character_card.allow_user_edit=false）。")
    ...
    async def _character_suggest(self, event, store, tokens: list[str], cfg: dict):
        """`/角色 补全 <角色名> [作品]`：用 danbooru 标签服务查候选标签（人工确认后落库）。"""
        _rest, _flags = self._pop_flags(tokens)
        if not _rest:
            await self._send(event, "用法：/角色 补全 <角色名> [作品名]\n查到的标签会用消息发给你，确认后再用 /角色 锚点 add 落库。")
            return
        ch = store.get_character(_rest[0])
        _name = (ch or {}).get("name") or _rest[0]
        _work = _flags.get("作品") or _flags.get("work") or ((ch or {}).get("work") or "")
        _res = await character.suggest_anchor(self, _name, _work)
        if not _res.get("ok"):
            await self._send(
                event,
                f"补全失败：{_res.get('msg')}\n（也可以直接 /角色 记住 {_name} <标签串> 手填）",
            )
            return
        _tags = _res.get("tags") or ""
        await self._send(
            event,
            f"「{_name}」候选标签（来源：{_res.get('source')}，查询词「{_res.get('query')}」）：\n"
            f"{_tags}\n"
            f"确认无误后落库：\n/角色 锚点 add {_name} 默认装 {_tags}\n"
            "（⚠️请人工核对：标签服务返回的可能包含不适用的服装/场景词，删掉不需要的再落库。）",
        )
    ...
    async def cmd_help(self, event: AstrMessageEvent):
        ...
        text = (
            ...
            "/角色 [列表 | 看 角色 | 记住 角色 标签串 | 改 角色 字段=值 | 锚点 add/set/use/del | 参考图 列表/记住/删 | 补全 角色 | 删 角色]\n"
            "    角色卡片：把「某角色长什么样」落库（一个角色可有多套锚点），画到时插件自动注入，避免每张图长相不一样\n"
    ...
    # 静默 return None 让框架终止 Agent Loop（v5.11.5 起不再代发收尾消息，
    # 图已发出、模型也已收尾过，沉默比机械话术体验好）。
    _DRAW_RUN_STOP_HINT = (
    ...
    async def llm_draw(
        ...
        0. 【全程静默，最重要】从决定画图到图片发出的整个过程，**不要输出任何过程性文字**——
           ...
           comfyui_workflows / danbooru 检索 / nai_codex / 记忆或知识库检索〔bot 自身入画时必查〕）
           一律静默调用，查完直接调用本工具出图。
        ...
        - 用户提到"用某个 LoRA/某种风格/画风/某个角色人物"→ 先调 comfyui_loras 查匹配项再填进 loras；不要因为用户没报名字就跳过，也别只让用户"给确切名字"而不动手。
        ...
        - N 张（"来 3 张猫"）→ prompts 传 N 条，每条写一个【不同】画面（不同姿势/场景/光线）。
        - "各来 X 张"（"白天、晚上、深夜各来 2 张"）→ prompts 传【组数 × X】条（本例 6 条）。
        - "来几张/多画几张"但没给数字 → 按 3 张，prompts 传 3 条。
        ★绝不要用「单条 prompt + count=N」表示 N 张——那只出同一画面的近似副本，插件会拦回。
        ...
        ★bot 自身入画（「你和XX的合照」「你的自拍」「画你自己」等）：「你」指 bot 的人格角色，
        它的外观锚点/绘图触发词**不在你的常识里，也常常不在会话上文里**——必须先【静默检索】：
        ①调用可用的记忆/知识库检索类工具，搜「<人格名> 绘图提示词 / 触发词 / 外观锚点」
        （如「小叽 绘图」「小叽 外观锚点」）；②或翻会话内之前画该角色用过的提示词。
        拿到锚点后原样用于该角色（多人合照时放进该角色的分组括号），并在后续轮次逐字沿用。
        ...
        **绝不凭常识编造 bot 自身的形象**（用户已投诉过"现在都不去找了"）。
        ...
        3) 某角色用了 LoRA：该 LoRA 的触发词必须写进**那个角色自己的分组括号**（插件检测到已存在
           ...
           ★★多人都用角色 LoRA（关键）：两个 LoRA 是同时全局作用于整张图的，仅靠触发词分组
           拉不开——每个角色的分组里必须再写 2~4 个该角色最有辨识度的外观标签
           （发色/瞳色/兽耳/角等）与本图服装，把两个角色"钉"在各自的组里；
           否则会出现两人同脸、A 的衣服穿到 B 身上（实测：双人图两个头都是薄荷、衣服都是鉴定师）。
        ...
        - krea2 / flux / qwen 等自然语言系：**英文自然语言整句**；不写任何 danbooru 标签与质量词
          （masterpiece / score_9 / very aesthetic 这类词会被当成"要画出来的文字"渲染到图上——
          这正是图上出现 score_9 等意外文字的第一大原因）；要渲染的文字放英文引号内写清。
        ...
        2) **再查 danbooru MCP（查标签的唯一正确来源）**：工具名可能中/英文，名字含 danbooru / tag / 标签 的 MCP 都属于它，**只调它**。
           ⛔禁止联网搜索 / 抓 danbooru 官网或镜像（限流 / 403、格式易错）。
        3) 仅当确认工具列表里没有任何 danbooru MCP 时，才用你自己的知识写标准标签（仍须遵守下一条）。
        4) 都不命中 → 不出图：直接告诉用户「没找到『XX』相关的 LoRA 或标签，没法画」，绝不凭记忆瞎编硬画。
        ★★拿到角色/作品 tag 后**禁止补外观**（最高频错误）：角色 tag（如 hatsune_miku、belle \\(zenless zone zero\\)）+ 作品 tag
        ...
        Args:
            ...
            workflow(string): 文生图工作流名，可选；不确定就留空用默认（可先调 comfyui_workflows）。仅文生图用，图生图请填 img2img_workflow。
                ★「用XX」消歧：XX 是平台（说了"平台"二字、点名平台名如 agnes/日日新/nai、或类型名 nai/openai/custom）→ 填 platform；
            ...
            count(number): 【恒为 1，一般不用传】张数由 prompts 条数决定；传大于 1 会被拦回并要求改用 prompts。
                （用户说"一张/单张"→ 1 条；"来 3 张"→ 3 条；"换个角度/再来"默认仍 1 张；"来几张"没给数字 → 3 条。）
            prompts(array): 多条出图项，**要几张就传几条**，每条各出 1 张（每条写【不同】画面，别重复同一条）。
                ① 字符串数组：["1girl 猫娘", "1boy 骑士"]（"白天、晚上、深夜各来 2 张"→ 6 条）。
                ② 对象数组（每项可独立定制）：{"prompt","workflow","img2img_workflow","loras","width","height","denoise","seed"}，
                   未写字段回落全局。例：要"真人、动漫各一张、各用各的工作流"→
        ...
        - 图生图参考图（★最容易出错）必须是**用户自己发的那张原图**；你（AI）上次生成的结果图**不算**参考图，
          除非用户明确说"把上次那张成品再改"。用户说"再改一下/继续改这张"（没发新图）时应基于**最初那张原图**继续改，
          找不到就请用户重发，不要拿最近生成的图顶替。群聊里无关旧图也不算。
        """
        ...
        if not _gate_ok:
            ...
            # 直接 return None 熔断 —— 框架对工具返回 None 会直接终止 Agent Loop。
            # v5.11.5 起不再由插件代发「（图片已经在上面发过啦…）」收尾语：
            # 图早已发到聊天窗口、模型前一次拦截（DONE_HINT）时也已自然收尾过，
            # 再补一条机械话术只会让用户觉得突兀（实测反馈体验差）。
    ...
    async def llm_character(
        ...
        try:
            ...
            if _act == "add_ref":
                ...
                _u = (ref_url or "").strip()
                if not _u:
                    return "add_ref 需要 ref_url（http(s) 图片直链，或本机绝对路径）。"
                _is_web = bool(re.match(r"^https?://", _u, re.I))
                _MAX = 12 * 1024 * 1024
                try:
                    if _is_web:
                        import aiohttp
                        async with aiohttp.ClientSession(
                            ...
                            async with _ses.get(_u) as _resp:
                                _resp.raise_for_status()
                                data = await _resp.read()
                        # 不引入 urlparse：去掉 query/fragment 后取文件名即可
                        _fname = (os.path.basename(_u.split("?", 1)[0].split("#", 1)[0]) or "ref.png")
                    else:
                        _p = Path(_u)
                        if not _p.exists() or not _p.is_file():
                            return f"本地路径不存在：{_u}"
                        data = await asyncio.to_thread(_p.read_bytes)
                        _fname = _p.name
                except Exception as e:
                    return f"读取参考图失败：{e}"
                if len(data) > _MAX:
                ...
                if ref is None:
                    return "参考图落地失败。"
                _n = float(ref.get("nsfw_score") or -1)
                return (
                    f"已为「{ch['name']}」存下参考图 id={ref['id']}"
                    + ("（同图已存在，复用记录）" if ref.get("dedup") else "")
                    + (f"，NSFW 置信度 {_n:.2f}" if _n >= 0 else "，NSFW 未检测（检测器不可用）")
                    + f"。路径：{ref.get('path')}"
                )
            if _act == "delete_ref":
                ch = store.get_character(name or "")
                if ch is None:
                    return f"没找到角色「{name}」。"
                if not ref_id:
                    return "delete_ref 需要 ref_id（先用 action=list_refs 查）。"
                ref = store.get_ref(int(ref_id))
                if ref is None or int(ref.get("character_id") or 0) != int(ch["id"]):
            ...
            if _act == "suggest":
                ...
                if not _res.get("ok"):
                    return f"补全失败：{_res.get('msg')}"
                return (
                    f"「{name}」候选标签（来源：{_res.get('source')}，查询词「{_res.get('query')}」）：\n"
                    f"{_res.get('tags')}\n"
                    "★这是候选：请人工核对后（删掉不适用的服装/场景词、补上角色专有 tag）\n"
                    f"用 action=save 或 add_anchor 落库（name={name}, positive=核对后的标签串）。"
                    "落库前必须让用户确认，不要直接写入。"
                )
            return (
    ...
    async def llm_loras(
        ...
        **务必先调用本工具**查询是否有匹配的 LoRA（可结合 keyword 或 category 缩小范围），
        ...
        def _fmt(l, verbose: bool) -> str:
            ...
            # verbose 仅对「精确查询（带了 keyword）且命中很少」开启：展开描述+触发词便于选 LoRA。
            # 否则（库大/无关键词/命中多）只列名称+分类+底模，避免把每个 LoRA 的描述+触发词全灌进
            # context，污染后续每一轮 LLM 推理（这是「出图前戏很久」的隐性元凶之一）。
            if verbose:
                desc = (l.get("description") or "").strip()
                if desc:
                    line += f"\n  描述：{desc}"
                tw = (l.get("trigger_words") or "").strip().replace("\n", " / ")
                if tw:
                    if len(tw) > 200:
                        tw = tw[:200] + "…"
                    line += f"\n  触发词：{tw}"
        ...
        # 先按过滤条件取命中条目（不带格式），再决定精简/展开与截断
        _hits = [l for l in lib if _match(l)]
        note = ""
        if not _hits and kw:
            _hits = [l for l in lib if _relaxed_hit(l)]
            if _hits:
                note = f"（未精确匹配到「{keyword}」，以下为忽略版本后缀/底模/分类后的近似结果）"
        if not _hits and wf_bm:
            _hits = [l for l in lib if _match(l, check_bm=False)]
            if _hits:
        ...
        if not _hits:
            ...
            return "没有可用的 LoRA。"
        # 体积控制：带 keyword 且命中很少（≤8）→ 展开描述/触发词；否则精简（仅名称/分类/底模）。
        # 命中超过 40 条截断，提示用 keyword/category 缩小。
        _verbose = bool(kw) and len(_hits) <= 8
        _MAX = 40
        _overflow = 0
        if len(_hits) > _MAX:
            _overflow = len(_hits) - _MAX
            _hits = _hits[:_MAX]
        rows = [_fmt(l, _verbose) for l in _hits]
        if note:
        ...
        if _overflow:
            head += f"（仅显示前 {_MAX} 条，其余 {_overflow} 条请用 keyword/category 缩小范围）"
        return head + "\n" + "\n".join(rows)
    ...
    async def llm_workflows(self, event: AstrMessageEvent):
        ...
        触发时机：在调用 comfyui_draw / comfyui_img2img 之前，如需确认
    ...
    async def llm_img2img(
        ...
        什么时候绝对不要调用（防误触发）：
        - 用户表示拒绝/取消/不需要（"不用画了""别画了""算了别改""取消""不要图""先别发""停""收回"）→ **绝对不调**，
          也不调任何发图工具，只用文字回应。
        - 普通闲聊、提问，无图生图意图 → 不调。
        - 判断只看用户**当前消息的明确意图**；历史里做过变换不代表现在还要做，别被之前请求带偏。

        重要约束：
        - 必须有参考图；当前消息没图、也定位不到用户最初那张原图时，提示用户先发一张再描述变换。
        - 用户再次表达改图意图（"再改一下""重新改""继续改这张"）→ 重新调用。
        - ★参考图选择（最容易出错）：参考图 = **用户自己发的那张原图**；你上次生成的结果图**不算**，
          除非用户明确说"把上次那张成品再改"。用户说"再改/继续改"（没发新图）时基于**最初那张原图**继续改；
          定位不到就请用户重发，别擅自用最近生成的图顶替。
        - 传 image（消息里图片的 URL）或让插件自动提取均可。★用户当前消息已附图时直接用，**不要**调 get_message_detail 之类
          回拉"原始消息"重新下载（签名 URL 本机常下不动，耗时且必失败）。当前消息里的图插件可直接用。
        ...
        Args:
            prompt(string): 【必填】基于参考图的变换/生成描述（中文或英文均可），直接给文本，别留空、别包裹 markdown。
            negative_prompt(string): 负向提示词，可选。
            ...
            image(string): 参考图 URL；多数情况插件自动从消息提取，无需传。
            denoise(number): 降噪/重绘强度（0~1）；不传或 -1=工作流默认。用户问"改多少/像不像原图"时传。
            count(number): 【恒为 1，一般不用传】张数由 prompts 条数决定；传大于 1 会被拦回。
            prompts(array): 多条出图项，**要几个效果就传几条**，每条各出 1 张（每条写【不同】变换）。
                ① 字符串数组：["转成水彩", "转成油画"]（"三个效果各来 2 张"→ 6 条）。
                ② 对象数组：{"prompt","workflow","img2img_workflow","loras","width","height","denoise","seed"}，未写字段回落全局。
                与 prompt 二选一，都传以 prompts 为准。
        ...
        if not _gate_ok2:
            # 与 comfyui_draw 同策略：硬终止信号直接 return None 熔断（防空转），
            # 不再回文本让模型继续空转；此时图已发出，静默结束即可。
            if _gate_hint2 is plugin._DRAW_RUN_STOP_HINT:
                return None
            return _gate_hint2
