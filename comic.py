...
# 风格锁：形状/位置/描边/配色仍交 LLM 按情绪现编，但【字体】必须锁定——
# 此前（v5.6.16）字体被一并交 LLM 现编，LLM 不写字体时 boogu 会退回系统默认
# 宋体/僵硬印刷体，实测「出图大概率默认字体、很丑」即由此而来，故在此兜底。
# 注意只锁「手写感/卡通感艺术字」这一大类，具体哪种（圆润卡通体/粗黑体/手写体/毛笔体）
# 仍由 LLM 按情绪与样式自选，不会退化成写死一种字体。
DEFAULT_BOOGU_SUFFIX = (
    "画面其他部分与参考图保持一致；添加的文字必须使用【手写感或卡通感的艺术字体】"
    "（如圆润卡通体、粗黑体、手写体、毛笔体），严禁使用系统默认宋体、细等线体或僵硬印刷体；"
    "文字清晰可读、无错别字。"
)
...
BOOGU_BUBBLE_CATALOG = [
    {
        ...
        "desc": "在人物头部右上方添加一个云朵形状的对话气泡，边缘圆润蓬松、底部一个小尖角指向人物的嘴；气泡底色用【半透明白或淡彩色】，描边颜色跟随角色发色或情绪（如害羞→粉、平静→浅灰褐），不必固定深棕。气泡内用圆润卡通体写简体中文「{text}」，字号随字数自适应（≤4字可大，5-8字中等，更多则缩小并自动换行），整体适中偏大、留出内边距确保文字不溢出气泡，横向居中。",
    },
    {
        "name": "圆角对话气泡",
        "tokens": ["圆角", "对话气泡", "说话", "speech"],
        "use_for": "普通说话、陈述、解释、提醒",
        "desc": "在人物头部一侧添加一个圆角矩形对话气泡，一条小尾巴指向人物；气泡底色用【半透明白】，描边颜色跟随情绪（平静→墨色、害羞→粉、开心→暖橙），不必固定深棕。气泡内用圆润卡通体写简体中文「{text}」，字号随字数自适应、整体适中偏大、留边距不溢出，横向居中。",
    },
    {
        "name": "思考气泡",
        "tokens": ["思考", "os", "内心", "悄悄话", "thought", "疑问", "问号"],
        "use_for": "内心想法、OS、悄悄话、犹豫、自言自语",
        "desc": "在人物头部上方添加一串由小到大的圆形思考气泡连向头部，最上方大气泡用圆润手写体写深灰色的简体中文「{text}」，字号中等偏大、线条细，表达内心想法；气泡可半透明。",
    },
    {
        "name": "爆炸气泡",
    ...
    },
    {
        ...
        "desc": "在人物头部一侧添加带尖锐棱角/锯齿边缘的对话气泡，描边颜色可用怒气红或深色；气泡底色用【半透明白】。气泡内用粗体写简体中文「{text}」，字号随字数自适应、整体适中偏大、留边距不溢出，表现怒气或急促。",
    },
    {
        "name": "底部字幕条",
        "tokens": ["字幕", "底部", "旁白", "caption", "说明", "总结"],
        "use_for": "【默认不加】仅旁白/吐槽/真相总结/用户明确要字幕时才用",
        "desc": "【默认不要加】只在吐槽/真相总结/用户明确要字幕时才用。若使用：在图片底部约六分之一处添加一条【半透明】圆角横条字幕，用粗圆润卡通体写简体中文「{text}」，带描边，字号随字数自适应、整体适中、横向居中，可爱但不抢眼。",
    },
    {
        "name": "无气泡白字黑边",
        "tokens": ["无气泡", "经典", "白字", "meme", "大字", "标题"],
        "use_for": "通用梗图、大字标题、上方/下方居中配字",
        "desc": "不使用气泡框，在画面合适位置（底部或顶部）用粗体写简体中文「{text}」，白色填充+深黑粗描边，字号尽量大、随字数自适应（字数多则缩小并换行、勿超出画面边缘），经典表情包大字、醒目清晰，横向居中。",
    },
    {
        "name": "无气泡直接压字",
        "tokens": ["压字", "直接", "叠加", "overlay"],
        "use_for": "把字直接压在画面安静区、不挡主体",
        "desc": "不使用气泡框，在画面较空区域用圆润卡通体写简体中文「{text}」，加白色细描边保可读，字号中等偏大、自然摆放；文字可半透明叠加在背景上不抢主体。",
    },
    {
        "name": "放射爆裂大字",
        "tokens": ["放射", "爆裂", "拟声", "冲击", "放射状", "speed"],
        "use_for": "拟声词（咚/啪/汪）、极度强调、冲击感",
        "desc": "在画面中央/人物附近用放射状爆发的粗体字写简体中文「{text}」，字形向外炸开带速度线，橙红配色加深色描边，字号尽量大、随字数自适应（勿溢出画面），表现冲击与强调。",
    },
]
...
def boogu_nl_hint(slot: dict) -> str:
    ...
    return (
        ...
        "★ 字号必须【随字数自适应、以清晰醒目为准、宁大勿小】：字数≤4 可大字醒目；"
        "5-8 字中等偏大；>8 字中等并自动换行。文字要在气泡/图形内【留出内边距、绝不溢出边缘】"
        "（溢出仍是严重错误），但在不溢出的前提下【尽量写大写醒目】，"
        "禁止把字缩成一小团，禁止用『很小/极小/细小』这类描述。\n"
        ...
        "（★必填：必须从 圆润卡通体/粗黑体/手写体/毛笔体 等艺术字里明确选一种并写出来，"
        "漏写字体或写成『默认字体』均视为错误）、字号（随字数自适应、宁大勿小）。"
        "文字务必短：气泡 ≤8 字、底部 ≤6 字。\n"
...
async def comic_write_prompts_llm(self, wf: dict, user_text: str, scene: str, subject: str = "user") -> dict:
    ...
    prompt = (
        ...
        "写详细英文 Danbooru 风格标签，开头固定加画质前缀 "
        "`masterpiece, best quality, ultra-detailed, highres, absurdres, intricate details, "
        "soft cinematic lighting, vibrant colors, refined anime style`，再写主体、表情、动作、构图；"
        ...
        "★ 字体必须写明，且只能用【手写感或卡通感艺术字】（圆润卡通体/粗黑体/手写体/毛笔体），"
        "严禁系统默认宋体、细等线体或僵硬印刷体；漏写字体视为错误。"
        "字号随字数自适应、宁大勿小，文字务必简短（气泡 ≤8 字、底部 ≤6 字），"
        "必须在气泡内留边距、绝不溢出（溢出是严重错误）。\n"
    )
    if _nl_notes:
        prompt += f"\n【boogu 约束】{_nl_notes}。"
    if _thought_note:
