...
新版是分页签结构（基础信息 / 提示词 / 尺寸 / 采样器 …），旧版是线性表单。
v7.7.44 的「关联的提示词 skill」下拉只加进了旧版那一份 → 新版工作流看不到这个选项。

- 新版表单的**「提示词」页签**补上「关联的提示词 skill」下拉（与旧版同一份选项与说明）；
- 旧版表单保留该下拉（旧版工作流同样受支持）；
- 顺带核对整条链路：后端读的是工作流条目自身的 `prompt_skill`（与是否引用基础工作流无关），
  前端列表直接来自配置、无字段过滤 → 新版/旧版都能生效。

注：顺手想在新版表单加「Anima 工作流」开关，核实后发现新版的标签系身份由基础工作流底模
**强制覆盖**（`wf["is_anima"] = _tag_style`），手动开关会被无声忽略，故未加（避免误导）。

## v7.7.44（工作流可「关联 skill」：每个工作流指定用哪套提示词规范）

工作流条目新增 **`prompt_skill`（关联的提示词 skill）**，比「按底模 + 意图自动判定」更明确、可控。

| 取值 | 行为 |
| --- | --- |
| `auto`（默认） | 按底模与提示词自动判定（沿用 v7.7.42/43：Qwen 系底模走 Qwen 规范；说到三视图/设定板走设定板规范） |
| `off` | 不用任何 skill 规范，回到原有逻辑（标签系改写 / 写实清理 / 翻译） |
| `qwen-t2i` | Qwen-Image **文生图**官方格式（约 20 句英文长段、不写比例与画质套话） |
| `qwen-edit` | Qwen-Image **图像编辑**规范（正文随指令语言、多图用 `<image1>`、只改点名属性） |
| `character-sheet` | 角色设定板规范（纯中文三视图/四视图设定板） |

- **显式指定不吃底模**：给非 Qwen 底模的工作流也能挂设定板/Qwen 规范；
- 触发口径：显式关联时**原生调用也一律生效**（纯英文也按规范走一遍）；`auto` 沿用旧口径
  （含中文才改写 / 设定板意图无条件改写）；
- `off` 时完全不碰 skill 规范（连 Qwen 系底模也不按官方格式改写）；
- 「第三方插件调用时用 LLM 整理」关闭时仍以该开关为准（原样使用调用方提示词）；
- WebUI：工作流编辑弹窗新增「关联的提示词 skill」下拉；列表卡片上非 `auto` 会显示
  `skill：xxx` 标签；插件配置页（AstrBot 配置）同样有该下拉。

测试：新增「关联 skill」用例组——显式指定（设定板/Qwen 文生图/Qwen 编辑/off）的解析、
非 Qwen 底模挂设定板生效、非法值回落 auto、端到端（显式 off 走原有清理、显式 qwen-t2i +
有输入图仍按文生图规范）。测试直接摘 main.py 里的真实解析函数执行，不复制一份逻辑。

## v7.7.43（Qwen-Image 画角色设定板：接入「角色设定板」规范）

用 **Qwen-Image 家族底模**画角色设定板（三视图 / 四视图 / 设定图 / 设定板 / 人设图…）时，
提示词改用**角色设定板规范**改写（纯中文专业设定板提示词）。

- **触发**：底模判定为 Qwen-Image 家族（见 v7.7.42）+ 提示词里有设定板意图
  （`设定板|设定图|三视图|四视图|多视图|正侧背|character sheet|turnaround` 等），
  第三方插件调用与原生调用都生效（原生纯英文也改写，因为意图已明确）；
- **规范来源**：`skills/character-sheet/character-design-sheet.md`（随包发布，**改文件即可换规范**），
  缺失时用内置精简规则。输出遵守该规范：左侧主视觉大立绘 + 档案 + 配色卡、右上三视图并排、
  右中面部/头部特写、右下细节小图横排、底部武器/纹理/装备/特效横条；白底 / 东方玄幻两模板；
  CG（默认 2.5D）/3D/真人三模式；角色 DNA 锁定（≥20 项，三视图严格一致）；**通篇中文、无英文标签**；
- **角色卡资料融入**：提示词里点名了角色卡（名字/别名）时，把该卡的**中文设定说明 + 已确认
  外观锚点**作为素材交给 LLM 一并写进设定板——锚点里的英文标签被明确要求**转写成中文**
  （而不是像以前那样事后把英文标签直接追加进提示词）；
- **设定板模式下跳过英文锚点标签注入**：设定板规范要求通篇中文，事后再追加英文标签会破坏它；
  角色资料已在改写阶段融入，所以把注入让开（日志会写明「跳过锚点标签注入」）；
- 设定板场景**不做英文翻译兜底**（该规范就是要中文）；改写失败就保留原文，绝不阻断出图；
  关闭「第三方调用时用 LLM 整理」时同样尊重配置，不改写、也不翻译；
- 给 LLM 的「可用工作流清单」与底模库里 Qwen 条目的说明同步补上「画设定板走设定板规范」。

测试：`tests/test_refine_fallback.py` 新增 **5 组**设定板分支（规范改写 / 失败保留中文 /
关闭整理不动 / 非 Qwen 底模不启用 / 原生英文仍改写）+ 设定板规范文档读取校验 + 角色卡资料
上下文校验（点名才带、英文标签须转写）。

## v7.7.42（Qwen-Image 接入官方格式提示词规范）

底模为 Qwen-Image 家族时，提示词改用**官方格式规范**改写（规范文档随包发布，可自行替换）。

- **规范来源**：`skills/qwen-image/qwen-image-t2i.md`（文生图）、`qwen-image-edit.md`（图像编辑）
  —— 两份文档本就是「可直接作系统提示词」的写法，插件直接把它作为系统提示交给 LLM；
  文件缺失时退回内置精简规则。改文件即可换规范，不用改代码；
- **两种模式自动分流**：无输入图 → 文生图规范（约 20 句英文长段、不写比例与画质套话、
  背景紧接开篇、方位短语 8–14 个、光照单独成句、画面内文字直双引号单语）；有输入图（图生图）
  → 图像编辑规范（只改点名属性、其余按输入图保真、正文语言随指令、多图用 `<image1>` 引用）；
- **触发**：第三方插件调用（整理开启时）按规范改写；原生调用（/draw 等）**含中文**时同样改写，
  纯英文不打扰。底模库「提示词风格」新增 **Qwen-Image（官方格式长描述）** 选项（内置 Qwen 条目
  已改为此项），未显式配置时按底模/工作流名含 `qwen` 自动识别；
- **回退链沿用 v7.7.41**：文生图正文必须英文 → 改写失败/无模型时回退**翻译**；图像编辑正文
  允许中文 → 改写失败保留原文。关闭 LLM 整理时，Qwen **文生图**含中文仍走翻译，编辑不动；
- 给 LLM 的「可用工作流清单」里，Qwen 底模的风格说明也换成了官方格式描述。

测试：`tests/test_refine_fallback.py` 扩到 **14 组**（新增 Qwen 文生图/编辑/失败回退/关闭整理/
原生改写）+ 规范文档读取校验（确认真读到 `skills/qwen-image/`，而非悄悄退回内置规则）。

## v7.7.41（第三方调用回退链细化：LLM 整理失败/关闭时，必须英文的工作流仍走翻译）

判定「必须英文」= 工作流底模为标签系（`is_anima`，Danbooru 系底模）——喂中文必然出废图。

1. **LLM 整理失败 → 回退翻译**：第三方调用 + 标签系时，改写无可用模型/超时/异常不再
   直接放行原文，而是回退走一次**翻译**（`_translate_prompt`），中文提示词照样变成
   英文标签；
2. **关闭 LLM 整理 ≠ 可以喂中文**：`third_party_llm_refine=false` 时，写实/自然语言系
   工作流原样使用（中文可直接用）；**标签系工作流若提示词含中文，仍自动翻译**——
   这条兜底不能省；
3. **平台链路同样对齐**：NAI（标签系平台）在整理关闭或整理失败时，含中文走翻译；
   OpenAI 兼容/自定义（自然语言系）不翻译；
4. 调度条件同步细化：第三方调用 + 关闭整理 + 无需英文时不再空转一次整理 task；
5. 新增 `tests/test_refine_fallback.py`：8 组覆盖（改写成功/回退翻译/关闭整理/纯英文
   原样/非标签系原样/原生调用/翻译失败保留原文），用 ast 摘真实函数 + fake self 跑。

## v7.7.40（新配置：可关闭「第三方插件调用时的 LLM 提示词整理」）

此前第三方插件（伴侣等）调用本插件画图时，提示词**必定**经 LLM 整理（Anima 工作流改写
为 Danbooru 英文标签、其它工作流清理为写实中文），只有超时配置、无法关闭。

- 新增 **`third_party_llm_refine`**（出图行为组，默认开）：关闭后第三方调用**原样使用**
  调用方传来的提示词——不调 LLM、不改写、不清理；原生指令（/draw 等）不受影响；
- ComfyUI 主链路与生图平台链路两处都已受控；日志会写明「已按配置跳过 LLM 整理」。

> 注：若连「提示词丰富化（LLM 扩写/画质前缀）」也想去掉，那是独立开关
> （配置项 → 提示词丰富化），与本次这项互不影响。
...
（绑基础工作流）的编辑弹窗「基础信息」tab 现在同样有该输入（0/留空 = 沿用全局）。

## v7.7.37（参考图上限改在工作流条目里配置）

- 出图工作流条目新增 **`max_refs`**（编辑弹窗「节点配置」区，参考图节点旁）：本工作流
  多参图生图单次最多取几张参考图（1~8）；0 或留空 = 沿用全局「出图行为 → 图生图参考图
  上限」（默认 3）；
- 全局 `img2img_max_refs` 保留，作为未单独配置的工作流的默认值。

## v7.7.36（WebUI：工作流列表卡片也显示「多参」标识）

基础工作流列表有「多参图生图（N 路）」标签，「工作流」页的卡片此前只在详情里体现——
现在图生图工作流绑定了多参基础工作流时，列表卡片在「图生图」旁也会显示「多参」小标签。

## v7.7.35（新功能：多参考图图生图——多图自动扩节点 + 图片选择器）

支持**多参图生图**工作流（条件节点带 `images.image_N` 动态输入，如 **Qwen Image 2.1
Edit**，提示词里的「图N」按顺序对应第 N 张参考图）：

- **解析器**：识别 `images.image_N` → roles 记 `multi_image` 标识（节点/路数），类型仍为
  图生图；WebUI 基础工作流列表显示「多参图生图（N 路）」标签；
- **注入**：用户一次发 M 张图时，插件**自动复制工作流里的 LoadImage + 缩放节点链**
  （如 `LoadImage → ImageScaleToTotalPixels`，缩放参数原样拷贝）扩出 M−N 路，按顺序接线
  `images.image_2..M`——单图与旧工作流行为完全不变；
- **引用图优先**：取图顺序改为「引用消息里的图 → 本条消息附带的图 → 兜底历史」
  （引用是主体、新发的是辅助；行为变化：引用 A 图 + 自发 B 图时单图工作流现在取 A）；
- **`--图` 图片选择器**（`/图生图` 与 `/draw`，别名 `--图片`/`--refs`/`--参考图`）：
  `--图 2` 取第 2 张、`--图 2,4` 第 2 和 4 张、`--图 2-4` 范围、`--图 -1` 倒数第一；
  序号对应提取顺序（引用图在前）；
- **数量上限**：新配置 `img2img_max_refs`（出图行为组，默认 3，1~8），超出取前 N 并提示；
- 新增 `tests/test_multi_ref.py`：解析器标识、扩节点（链复制/接线/参数拷贝/ID 不冲突）、
  选择器、硬件名精简等五组测试。
...
1. **硬盘显示剩余空间**：「剩余 剩余GB / 总量 GB」+「已用」（free 字段缺失时按 总-已 推算）；
2. **系统节不显示主机名**，改为：`系统`（`host.os.caption` + `display_version`，如
   「Windows 11 专业工作站版 25H2」）+「已运行」时长；
3. **CPU 温度接文档真实字段**：`host.thermal.cpu_package_c`（CPU 封装温度——Windows 上
   需中转站装 LibreHardwareMonitor 才有值，纯 WMI 拿不到时该字段为 null、行自动隐藏）。

## v7.7.30（绘图状态：边框跟主题、设备信息精简与字段容错）

1. **节边框跟随主题**：区块描边从固定灰改为「主题强调色混底色」；
2. **「风扇」节换成「系统」**（放设备首位：主机 / 系统 / 已运行时长）——风扇转速
   中转站基本不提供，拿不到就一直空着没意义；
3. **显卡拆行展示**：型号 / 利用率 / 显存 / 温度 各一行（之前挤在「状态」一行）；
4. **硬件型号精简**：`12th Gen Intel(R) Core(TM) i7-12700F` → `i7-12700F`、
   `AMD Ryzen 9 7950X 16-Core Processor` → `Ryzen 9 7950X`、
   `NVIDIA GeForce RTX 4090D` → `RTX 4090D`（`_short_hw_name`，CPU/GPU 各一套规则）；
5. **CPU 字段容错扩充**：型号/占用/温度/核数按多组候选字段名读取（host.cpu.*、
   cpu_name、processor…），中转站版本不同字段名可能不同；`/device` 原始响应会
   落盘到插件数据目录 `device_sample.json`——若某行仍显示不出来，把这个文件发
   维护者即可精准适配。

## v7.7.29（报表卡改版：图标徽章 + 4 列网格；/绘图状态 按 mockup 重排）

**报表卡渲染器**（`draw_card.render_report`，/绘图统计 等同步受益）：

- 每节标题带**图标徽章**（主题色圆角块 + 白色线性图标：CPU 芯片/显卡/内存条/风扇/
  硬盘圆柱/ComfyUI 方块/队列/对勾/奖杯/柱状图，自绘几何图形，不用 emoji 字体——
  seguiemj 小字号会糊成色块）；
- 节进 **4 列网格**：`sections[]` 新增 `span`（跨 1~4 格，缺省 0=通栏单列，旧数据完全
  兼容）、`icon`（显式指定图标，缺省按标题关键字猜）；**同行节等高**；
- 行 label 左对齐、值右对齐、**anchor 同基线**（旧版 y 偏移不一致导致的歪斜）；
- 右上胶囊改**不透明**强调色混白底——RGBA 上画半透明会丢 alpha 变纯色块、吞掉同色文字。

**/绘图状态**（按确认的 mockup 重排）：

- 只探「当前默认服务器」，状态与延迟进右上角胶囊（`正常 · 23ms` / `🔴 不可达`），
  不再列出所有服务器；
- 中转站设备拆五块：**CPU（型号/占用/温度）、显卡（型号/利用率/显存/温度）、内存、
  风扇（转速）、硬盘**——字段全部容错读取，中转站没提供的整节跳过（CPU 温度/
  风扇/硬盘容量取决于中转站 `/device` 是否给出）；
- 去掉 CosyVoice 状态（暂不展示）；「生图限额」改为「**生图统计**」（今日/昨日/累计，
  不再展示限额配置与管理员豁免）；
- **热门工作流 TOP3**（今天，含平均耗时）与生图统计并排一行。

## v7.7.28（重写 /绘图状态：接入中转站 TaskHub 辅助接口）

服务器若是中转站（TaskHub），`/绘图状态` 现在会调用它的 **`GET /device`**（本机 + 上游
一次采样，8s 超时、失败静默跳过），卡片在原有「服务器 / 生图限额」之外新增：

- **设备状态**：显卡型号、利用率、显存（可用/总量）、温度、功耗；CPU 占用、内存
  （多卡只展示第 1 张并注明总数，缺项字段自动跳过——老卡没有风扇转速等不报错）；
- **ComfyUI**：是否可达、版本、上游队列（执行中/排队）；
- **CosyVoice**：是否可达、模型是否加载、真实采样率、音色数；
- **调度队列**：排队/运行中、有效并发（GPU 过载自动降并发后的值）、累计入队/完成；
- **上游自检**：自检等级与异常条数，warn 以上列前 3 条（如显存过低、prompt 丢失）。

其余改动：

- **去掉「管理员豁免」**——生图限额一节不再展示该行（内部权限细节，不该对外）；
- 直连 ComfyUI（非中转站）的服务器没有 `/device`（404），按原样只展示服务器状态，零影响；
- 文字版同步：服务器行下方缩进展示设备摘要与 ComfyUI/CosyVoice 状态。

## v7.7.27（修 Bug：放大/抠图 NSFW 群白名单 + 参考图归档缺失 ＋ 放大倍率/护栏细化）

### 修 Bug

1. **放大/抠图的 NSFW 拦截不认「允许发 NSFW 的群白名单」**：出图与取图链路一直有查
   `_nsfw_group_allowed`，放大/抠图两处漏了——白名单内的群也被拦。已对齐。
2. **图生图的参考图在图库显示缺失**：放大/抠图只对 temp 输入图算哈希写进 `ref_sha256`、
   从不归档输入图 → ref 指向不存在的记录，大图的参考图永远加载失败。现在先把输入图
   归档成**用户图**（带 user_id 归属），ref 指向真实记录；归档失败时退回算哈希（留日志）。

### 图片放大 · 倍率与护栏细化（v7.7.27）

1. **倍率支持小数**：允许列表、默认倍率、指令参数全程按数值处理——允许列表填
   `1.5,2,2.5,3` 即可；只支持整数倍率的工作流不填小数即可。指令写法 `1.5x` / `x2.5` /
   `2.5倍` / `--倍率 2.5`；**裸数字位置参数只认整数**（裸 `2.0` 这类歧义 token 留给
   工作流名，如「VOSR 2.0」）。卡片/日志显示自动省去多余的 `.0`。
2. **回退原则细化（就近向下优先）**：请求倍率不在允许列表里时，选 **≤ 请求值的最大
   允许倍率**（2.7×→2.5×、5×→4×）；列表全都更大时取最小值；尺寸护栏降档同样优先在
   允许列表内选（放不下 3× 就先试 2.5× 再试 2×）。
3. **尺寸护栏按条目覆盖**：每个放大条目可单独配护栏档位（8g/12g/16g/自定义四个上限），
   留空沿用全局——不同工作流显存开销不同时各配各的。
4. **倍率字段手动指定（进阶）**：条目里可手动填「节点 ID + 输入框名」覆盖解析器的自动
   识别（解析不到或识别错时救场），两项都填才生效。

## v7.7.26（图生图卡片尺寸：输入与输出分开显示 + 参考图读取失败留痕）

处理中卡的「尺寸」从 v7.7.22 起就是**输入图（参考图）尺寸**（图生图不注入宽高，
尺寸由参考图决定）。这轮补两处让人不再困惑：

1. **结果卡区分「输入」与「尺寸」**：结果卡显示的是**实际输出尺寸**（工作流内部有
   放大/缩放时 ≠ 参考图尺寸，这是真实值不是显示错误）。图生图的结果卡现在会多一个
   「输入 832×1216」胶囊与「尺寸（输出）」并排，一眼看清两者关系；
2. **参考图尺寸读取失败留痕**：此前 Pillow 读参考图失败是静默回退（卡片退回显示
   工作流配置尺寸、看不出原因），现在打 warning 日志（含路径与原因）。

## v7.7.25（新增：结果卡关闭时的文字小结也可关闭 —— 完成后只发图）

「出图卡片」配置新增 **`result_text`**（默认开）：关闭「出图完成发结果卡」后，三条链路
（出图 / 放大 / 抠图）会用一行文字小结兜底；不想要这条文字的人把它也关掉即可——
**完成后只发图片本身，不再有任何附加消息**（处理中卡、失败卡不受影响）。

## v7.7.24（修 Bug：放大/抠图卡片右上角排队信息 + 抠图结果卡不认开关）

1. **处理中卡右上角恢复「排队 N」**：放大/抠图此前写成「有排队才显示排队、否则显示
   倍率/步数」，与出图链路（v7.4.6 起恒显「排队 N」，0 也显示=空闲一目了然）不一致。
   现在三条链路统一恒显「排队 N」；倍率/步数本来就在参数胶囊里，不占右上角。
2. **抠图结果卡不认「出图完成发结果卡」开关**：放大结果卡一直有先查
   `_card_would_send(event, "done")`，抠图漏了——关了开关抠图还是发卡（放大退文字，
   行为不一致）。已对齐：开关关闭时抠图同样退一行文字小结。

## v7.7.23（更多功能弹窗表单补字段间距）

弹窗表单是 `:show-feedback="false"`——naive-ui 的字段间距靠 feedback 占位区撑着，
关掉后 form-item 自身没有 margin，字段全贴在一起（竖排说明后尤其明显）。

- 普通字段加 `margin-bottom: 18px`；栅格内改用 `n-grid` 的 `:y-gap="14"`（清零内部
  margin，避免叠加）；说明文字与控件的距离不变。

## v7.7.22（修 Bug：图生图标识全显示成「文生图」+ 图生图卡片尺寸改用输入图 + 图库大图显示参考图尺寸）

1. **修「工作流」页类型标识**：新版条目（绑基础工作流）没有 `image_node` 字段，
   列表/详情的「图生图/文生图」判定只认旧版字段 → 新版全显示「文生图」。
   改为旧版认 `image_node`、新版看绑定的基础工作流解析出的 `roles.kind`。
2. **图生图信息卡片的尺寸**：图生图输出尺寸由参考图决定（插件本来就不注入宽高，
   `v7.0.18` 行为），卡片却显示工作流配置的默认宽高（没配时兜底 512），有误导。
   改为图生图时**直接显示输入图（参考图）尺寸**（`_in_w/_in_h`）；文生图行为不变。
3. **图库大图看参考图尺寸**：大图查看器的「参考图」标签旁现在会显示参考图尺寸
   （如 `参考图 1024×1536`）。`gallery/image?meta=1` 本来就返回图库记录（含 w/h），
   前端之前只取了 `data_url` 把 meta 丢了；点击参考图/结果图交换主次时尺寸跟着走。
...
1. **修 `/抠图` 必炸的 NameError**（线上报错）：v7.7.16 把抠图改条目式时参数从 `wf_spec`
   改名 `spec`，函数体里漏了一处——`compileall` 不报、一跑就炸。已修复，并新增
   **pyflakes 静态检查测试**（`tests/test_static_lint.py`，拦截 undefined name 级别的硬伤，
   这类错误编译期抓不到）。
2. **GIF 取第一帧接入「图片放大」与「抠图」**：`_gif_to_first_frame` 此前只接在图生图链路，
   用户给 /放大、/抠图 发 GIF 会把动图原样上传。现在两条链路与图生图同一处理：GIF 先取首帧
   转静态图再上传/归档（无 Pillow 环境降级原样上传）。
3. **顺手清掉静态检查扫出的 4 处存量雷**（都是特定分支/错误处理路径里才炸的）：
   - `_serialize_loras_text` 漏了 `self`（loraon/loraoff 持久化一调就 TypeError）；
   - 图生图「上传参考图失败」的失败卡引用了不存在的 `_size`（报错时二次崩溃）；
   - `/图库 搜索` 的结果行用了未定义的 `is_admin`（检索路径必炸）；
   - WebUI 两个日志回退点用了不存在的 `_log`、分享令牌作废路由调了不存在的 `_read_json_body`。

验证：pyflakes 全仓清零（11 个核心文件）；全套回归通过。

## v7.7.19（修布局根因 + 列表可直接设默认 + 删除加确认）

三个反馈：

1. **弹窗里的字段说明（hint）显示在控件右边**——根因：naive-ui 表单项内容区
   （`.n-form-item-blank`）默认**横向 flex**，控件和说明被排成一行。v7.7.17 只调了宽度与分组，
   没打中这个根因，抱歉。现在在「更多功能」与「基础工作流」两处弹窗里强制内容区**竖排**
   （控件在上、说明在下），一处 CSS 修掉所有表单项。
2. **「默认」标识在列表里看不到/不好设**：条目卡片新增**「设为默认」按钮**（绿色，仅图片放大、
   非默认的条目显示），点了立即生效并保存；默认条目显示绿色「默认」标签。（表单里的开关保留。）
   如果之前开了开关列表也没显示标签：请**更新插件后强制刷新页面（Ctrl+F5）**——浏览器缓存旧
   前端产物也会造成这类"改了没生效"。
3. **删除功能没有确认**：现在点「删除」先弹确认框（提示删除后对应指令不可用、绑定的工作流不受
   影响），确认后才删。

纯前端改动；vite build 通过。

## v7.7.18（图片放大条目：选工作流自动填内置倍率 + 「设为默认」开关，去掉 vosr2 默认名）

三个反馈一起修（都是「更多功能 → 图片放大」条目表单）：

1. **选了放大工作流后表单没被填充**：现在换绑工作流时会把**工作流内置倍率**自动填进
  「默认放大倍率」——优先取可写倍率字段的当前值（SeedVR2 的 `resize_type.multiplier=2`、
   VOSR2 连的 `easy int`），其次从放大模型名识别（`4x-UltraSharp.pth` → 4×）；
   字段说明里会显示「工作流内置：4×」供对照。`allowed_scales` 不自动填（容易误约束）。
2. **不要默认叫 vosr2**：新建条目的名称默认改为「图片放大」（抠图条目是「抠图」），
   占位符同步；schema 的 `name` 默认值也从 `vosr2` 改为 `图片放大`。
3. **缺「默认放大工作流」开关**：图片放大条目新增 **「设为默认放大功能」** 开关
   （「基本」区，启用旁）——发 `/放大` 不点名时**优先用设为默认的那条**（没设默认回落
   第一个启用的，行为向后兼容）；保存时自动保证**所有图片放大条目里只有一条默认**
   （把另一条设为默认会自动取消这条的），条目卡片上显示绿色「默认」标签。

验证：`test_image_upscale.py` 补 3 组默认优先级断言（默认优先 / 默认条目停用回落 / 点名最优先）；
schema 校验、全套回归、前端 vite build 通过。

## v7.7.17（优化「添加/编辑功能」弹窗的表单布局）

- 弹窗给足宽度（`min(760px, 94vw)`）——此前没有设宽，两列网格在窄弹窗里全被压成单列、
  字段排布参差；「＋ 添加功能」的功能选择面板单独收窄到 `min(520px, 94vw)`；
- 表单分区：**基本**（功能名称 + 启用并排一行、绑定的工作流）/ **放大参数**或**抠图参数**
  （带小节标题虚线分隔）；
- 所有输入控件统一 `width: 100%`（此前超时是 220px、其余 100%，宽窄不一）；
- 「种子策略」的说明挪进自己的表单项（此前悬在两个网格之间）；「固定种子值」也补了说明；
- 标题行下加功能类型标签 + 指令用法一句话；表单关掉校验占位（`show-feedback=false`）更紧凑；
- 抠图的「不放大」开关行改为开关 + 行内说明的对齐排布。

纯前端样式/结构调整，不改任何行为；vite build 通过。

## v7.7.16（抠图改为「＋ 添加功能」条目式，不再页面预置）

**反馈**：抠图不该在「更多功能」页里预置一块固定表单，应该和图片放大一样——点右上角
**「＋ 添加功能」**，在功能类型里选「抠图（去背景）」新增一条、各自配置、单独启用/禁用。

### WebUI（「更多功能」页）

- 移除预置的抠图卡片；**「＋ 添加功能」面板新增「抠图（去背景）」选项**（✂️），
  选定后进配置弹窗：名称 / 启用 / 绑定的抠图工作流（下拉，只列「解析通过 + 带图输入」的）/
  **不放大** / 步数 / 提示词 / 超时；
- 条目列表按类型渲染：图标（🔍 / ✂️）、类型标签、参数摘要（放大=倍率/允许倍率/种子；
  抠图=步数/提示词/不放大）、绑定校验标签（绑错类型/没图输入/解析失败都会标红说明）；
- 编辑弹窗按类型显示不同字段；抠图换绑工作流时仍会**自动把工作流的步数与提示词填进表单**，
  并显示工作流当前值（含预处理缩放节点的目标像素）；
- 兼容迁移：`features` 里没有抠图条目、但存在 v7.7.13~v7.7.15 的单条 `matting` 配置时，
  自动折算成一条并提示「保存一次完成迁移」。

### 后端（与图片放大同一套条目机制）

- `_matting_entries()`：从 `features` 里取 `kind=matting` 的条目（含旧配置折算）；
- `_resolve_matting_entry(spec)`：指令点名（功能名 / 条目 key / 绑定的工作流名·ID）> 第一个启用的；
  全部停用、点名不存在、没配置条目都给明确文案；
- `_matting_base_of(entry)`：解析条目绑定的基础工作流（校验：解析通过 + 带图输入 + 不是放大类）；
- `_do_matting` / `/抠图` 改为按条目取参数（步数 / 提示词 / 不放大 / 超时），卡片与日志带上功能名；
- `_conf_schema.json`：移除独立 `matting` 配置块，`features` 模板的 `kind` 增加 `matting` 选项、
  并补上 `steps` / `prompt` / `no_upscale` 三个字段（标注「仅抠图用」）。

### 验证

`tests/test_matting.py` 重写为条目式（8 组）：解析 / 条目读取（含旧配置折算）/ 挑条目（点名·停用·
不存在·无条目）/ 条目绑定校验（放大类·无图输入·解析失败·多候选）/ 注入 / 不放大 / 真实入库链路 /
指令别名。全套回归通过；compileall、schema 校验、前端 vite build 通过。

## v7.7.15（修 Bug：连传两个工作流时后一个会把前一个覆盖掉）

**反馈**：上传基础工作流时，点「解析并入库」后弹窗不关闭；再选另一个文件点入库，
**会把上一次入库的那条覆盖掉**。

原因（两处叠加）：

1. **前端**：入库成功后弹窗没关、表单没清空，且选文件时是「名称框非空就不更新」——
   第二次选文件仍沿用**上一个文件的名字**；
2. **后端**：入库按**显示名**撞名判定，撞上就 `UPDATE` 覆盖，而上传路由又把
   「已覆盖更新同名基础工作流」这句提示丢掉了 → **静默覆盖**（连同那条记录的封面、底模关联一起冲掉）。

### 修复

- **前端**：「解析并入库」成功后**关闭弹窗并清空表单**；选文件时按「名称是空的、或还是上次
  自动填的那个（= 用户没手改）」刷新名称；并把**真实文件名**一起提交（此前恒为空串）；
- **后端**：`WorkflowStore.import_json_ex()` 明确区分两种情况并返回 `info`：
  - **同名 + 同一文件名**（或两边都没有文件名）→ **覆盖更新**（重复上传同一文件的新版本，
    粘贴 JSON 更新同名条目的老用法也照旧）；
  - **同名但明显是另一个文件**（两边文件名都非空且不同）→ **新增**，名字自动去重成 `xxx (2)`，
    旧记录不受影响；
  - 上传接口返回 `updated` / `renamed` / `name`，WebUI 据此提示
    「已覆盖更新同名工作流「x」」（黄色警告）或「同名工作流已存在，已新增为「x (2)」」；
- `import_json()` 保留为兼容包装（老调用方仍拿 `(id, roles, error)`）。

### 验证

`tests/test_workflow_v7.py` 新增第 15 组：同名同文件 → 覆盖更新（id 不变）；同名不同文件 →
新增且名字去重、旧记录的文件关联与内容不变；粘贴 JSON（无文件名）同名 → 仍覆盖更新；
历史记录 `file_name` 为空 + 同名新上传 → 覆盖更新（不产生重复）；兼容包装仍返回三元组。
全套回归通过、compileall 与前端 vite build 通过。

## v7.7.14（抠图新增「不放大」：小图不再被预处理缩放插值放大）

**背景**：抠图工作流里常加一步「缩放图像（像素）」（`ImageScaleToTotalPixels`，如
`megapixels=1.25`）来统一工作分辨率。它按**总像素**归一化，不分大小一律缩放——**小图也会
被插值放大**到目标像素：输出比输入大，细节并不会变多（边缘反而可能更软）。

### 1. 「更多功能 → 抠图」新增开关「不放大（小图保持原尺寸）」（默认开）

- 规则：**目标像素 = min(输入图像素, 工作流原目标)**
  —— 小图保持原尺寸（只按边长对齐微调，最多差几十像素），大图仍按原目标缩小
  （显存保护完全不变，6000×4000 依旧被压到 ~1.25MP）；
- **输入本来就 ≥ 工作流目标时不碰节点**（工作流自己会缩，改写无意义，也避免把 INT 字段写成浮点）；
- 关掉 = 完全照工作流原值跑（一个字节都不碰）；工作流没有这类节点时本项自动无效；
- 处理中卡片会多一个「预处理 0.26MP（未放大）」胶囊，结果卡的「抠图」一节会多一行
  「预处理缩放 0.26MP（未放大）」，日志同步写明 `不放大：输入 0.26MP → 目标 0.26MP（工作流原目标 1.25MP）`。

### 2. 解析器：新增「预处理缩放」注记

- `_find_pre_scale`：从图输入节点**顺流 BFS**，找到离它最近的「类名像缩放（scale/resize）+
  带目标像素字段」的节点，记为 `roles.pre_scale = {node, field, default, unit, default_mp,
  class_type, steps_field, steps_default}`；
- 只认「总像素」类字段（`megapixels` / `max_total_pixels` / `total_pixels` / `target_pixels`），
  并按单位统一换算成 MP（`megapixels` 是 MP，其余是绝对像素）；`width`/`height` 类的普通
  `ImageScale` 不会被误判；
- 纯增量注记：没有这类节点时不产生该键，既有工作流的解析结果逐字不变。

### 验证

- `tests/test_matting.py` 新增第 5 组（解析出 `pre_scale` 注记；小图 512×512 → 写 0.26MP、
  800×1200 → 写 0.96MP；输入 ≥ 目标（3000×2000 / 1024×1536）→ 不碰节点；关掉「不放大」→
  不碰；尺寸读不到 → 不碰；无该节点 → 不报错；px 单位字段回写为**整数**）；
- 全套回归通过（`test_workflow_v7` 14 组、`test_image_upscale`、`test_draw_card`、
  `test_prompt_boost`、`test_prompt_guard`、`test_force_draw` 等）；
- compileall、schema 校验、前端 vite build 通过（产物已含「不放大」开关）。

用法：把带预处理缩放的抠图工作流**重新上传**一次（roles 是上传时解析的，「重新解析」不会读磁盘新文件），
再到「更多功能 → 抠图」确认「不放大」是开的即可。

## v7.7.13（新功能：抠图——带提示词的编辑/抠图工作流 + 「更多功能」新增抠图配置区）

**需求**：把「抠图 / 去背景」工作流（如 Qwen Image 2.1 Edit：提示词 `Remove the background, and
output a PNG image`）做成独立功能——用户带图发 `/抠图` 就能去背景，**不用传任何参数**；
步数与提示词可配置，**默认取工作流里的值并自动填进表单**。

### 1. 新指令 `/抠图`（别名 `/抠像`、`/去背景`、`/去背`）

- 取图与出图/放大同一套：消息内图片 / 引用消息里的图 / 本插件最近生成的那张；
- 执行链路与出图**完全独立**：只写「输入图 + 步数 + 提示词」三处——宽高、LoRA、采样器覆盖、
  提示词丰富化一概不碰（抠图的构图与分辨率由工作流自己决定：Qwen Edit 系拿参考图编码出的
  latent 当输入，硬改宽高反而会把构图弄坏）；
- 权限总闸（白名单/黑名单）、生图限额、NSFW 护栏、图库归档、配额计数、自动撤回与出图同一套；
  归档走 `os.replace` 原样搬运，**PNG 透明通道不会被重新编码丢掉**；
- 等待上限用本功能自己的 `timeout`（默认 300 秒），不套出图那套「单张等待硬上限」；
- 结果发**报表卡**（输入 / 输出 / 步数 / 耗时 + 提示词 + 工作流），画不出来退回一行文字；
  处理中卡写明输入尺寸、步数、工作流与提示词。

### 2. 「更多功能 → 抠图」配置区（单条配置）

- 字段：启用 / **绑定的抠图工作流**（下拉，只列「解析通过 + 带图输入」的工作流）/
  **采样步数** / **提示词** / 等待超时；
- **默认值取工作流里的**：选定工作流时**自动把工作流的步数与提示词填进表单**，并显示工作流的
  当前值供对照；另有「从工作流填充步数与提示词」与「清空（用工作流原值）」两个按钮；
- 步数 / 提示词**留空 = 沿用工作流原值**（后端只在配置有值时才写入），所以什么都不填也能直接跑；
- 绑错会当场说清：绑到**放大类**、绑到**没有图输入**的工作流、绑定的已删除或解析未通过，
  都给出针对性文案；库里有多个候选且未绑定时提示去指定。

### 3. 解析器：roles 记录「工作流当前提示词」

新增 `roles.positive.default_text` / `roles.negative.default_text`——工作流里现在写着什么，解析时
就记下来，供「更多功能」表单预填。纯增量字段，不影响任何既有判定。

### 验证

- 新增 `tests/test_matting.py` 5 组：抠图工作流解析（图输入 / 采样器 / 提示词与步数默认值）、
  挑工作流（ID / 名字 / 包含 / 唯一 + 放大类·无图输入·解析失败·无候选·多候选六种失败文案）、
  注入（步数 → 采样器、提示词 → 正向节点，**留空不动**，无注记不报错）、
  真实入库链路（上传 → 预填数据可用 → 挑中 → 注入，并断言模型/CLIP/VAE 资源节点没被动过）、
  指令别名与参数剥离（6 组）；
- `tests/test_workflow_v7.py` 两处精确断言同步补上 `default_text`；
- 全套回归通过；compileall、schema 校验、前端 vite build 通过（产物已含抠图配置区）。

用法：① 到「基础工作流」页上传抠图工作流（如 `qwen2.1抠图.json`，解析通过后类型显示「图生图」）；
② 「更多功能 → 抠图」打开开关、选它（步数与提示词会自动填好）、保存；③ 发图 + `/抠图`。
...
图片放大复用它时，卡片上会写成「vosr2 · 图生图」，与放大链路不符（也会让人以为走了重绘）。

- `_card_fail_info` 新增可选参数 `flow`，可覆盖副标题的流程名（**缺省行为不变**，出图链路不受影响）；
- 图片放大的三处失败卡统一传 `flow="{功能名} · 图片放大"`：尺寸护栏拦输入、连 1× 都超上限、
  以及 `_fail` 统一出口（上传失败 / 超时 / 无输出图 / ComfyUI 报错）。

验证：用 `draw_card` 真实渲染三张卡片核对文案——输入超限失败卡、处理中卡（含「护栏降档」胶囊）、
结果卡（含「尺寸护栏」一节）；compileall 与全套回归通过。

## v7.7.10（尺寸护栏档位改按显存命名，默认改成 8G 档）

**背景**：主力卡的显存是 8G，而 v7.7.9 的默认「标准」是按 8~12G 定的（输入 ≤3072，会拦 4K 输入），
对 8G 卡偏松。这版把默认值对准 8G，档位名也直接按显存标注，一眼就能选。

- 档位改为：**`8g`（默认）** / `12g` / `16g` / 不限制 / 自定义；
- **默认档 = 8g**：输入 ≤ 长边 2048px、≤4MP；输出 ≤ 长边 3072px、≤8MP —— 装完即用，不用配；
- **旧档位名自动映射**（v7.7.9 已保存的配置不会跑偏）：`strict → 8g`、`standard → 12g`、`loose → 16g`；
- WebUI 的档位下拉与对照表同步改名（并高亮当前档位），`_conf_schema.json` 的 options/hint 同步。

**8G 档下的实际表现**（护栏自动生效，卡片会写明降档原因）：

| 输入 | 请求 | 结果 |
| --- | --- | --- |
| 832×1216（1MP） | 3× | 3× 会到 2496×3648（超 3072 长边）→ **自动 2×** |
| 1024×1536 | 3× / 4× | 都超 → **自动 2×** |
| 2496×3648 | 4× | 连 1× 都超 → **拒绝**（提示先缩小图片） |
| 3840×2160（4K 照片） | — | **直接拦下**（长边 3840 > 2048），提示缩小图片或换 12g/16g 档 |

> 小提示：8G 卡上把某条功能的「默认倍率」直接设成 2，就不会每次都看到「护栏降档」提示；
> 留着 3 也没问题，护栏会自己降并说明原因。

验证：`tests/test_image_upscale.py` 第 10 组扩成「8g / 12g / 16g + 旧名映射」，用例覆盖各档位下的
放行 / 拦截 / 降档边界（含 8G 档「连 1× 都超 ⇒ 拒绝」的分支）、文案与「允许倍率回落 → 再按尺寸降档」
两层规则不打架；全套回归通过；schema 校验与前端构建通过。

## v7.7.9（图片放大：尺寸护栏 + 自适应倍率，全部带卡片提示）

**需求**：① 传入图太大要拦住；② 按传入尺寸自动限制最大倍率，别把显存打爆；③ 两者都要有卡片提示。

**新增全局配置 `upscale_limits`**（WebUI「更多功能」页顶部「尺寸护栏（全局 · 防爆显存）」）——
显存是机器级别的，所以做成**全局**设置而不必每条功能配一遍；**默认「标准」档，装完即用**：

| 档位 | 输入上限（超过直接拒绝） | 输出上限（决定自适应倍率） |
| --- | --- | --- |
| 严格（8G 显存以下） | 长边 ≤2048px、≤4MP | 长边 ≤3072px、≤8MP |
| **标准（8~12G，默认）** | 长边 ≤3072px、≤9MP | 长边 ≤4096px、≤12MP |
| 宽松（16G 显存以上） | 长边 ≤4096px、≤16MP | 长边 ≤6144px、≤24MP |
| 不限制 | 不限 | 不限 |
| 自定义 | 四个数值自填（0 = 不限制该项） | 同左 |

口径用「长边 + 总像素」：总像素比长边更能反映扩散式放大（VOSR2 / SeedVR2）的显存占用。

**执行顺序**（都在上传之前判断，省掉无谓的读盘与上传）：

1. 先读输入尺寸 → 命中**输入上限**：直接拒绝并发**失败卡**，写明图片尺寸、超了哪一项、当前档位与
   上限，并给出「先把图缩小 / 把档位调高」的建议；卡片信息行同时给出「护栏档位」与「输入上限」；
2. 否则按**输出上限**把倍率往下压（want → want-1 → … → 1）：
   - 发生降档时：**处理中卡片**多两枚胶囊「护栏降档 4×→2×」「输出上限 …」，**结果卡**多一节
     「尺寸护栏」（自动降档 / 输出上限），卡片关闭时的一行文字小结里也会写明；
   - 连 1× 都超上限 → 失败卡说明原因（例：输入 1024×1536 请求 3× → 自动用 2×；
     输入 2496×3648 请求 4× → 只能 1×）；
3. 无 Pillow / 读不到尺寸 → 跳过护栏（不阻断）；档位选「不限制」→ 完全按用户给的倍率走。

日志一条到底：`【放大】 尺寸护栏：输入 1024×1536｜档位 标准（8~12G 显存，默认）（输入 长边 ≤3072px、
总像素 ≤9MP／输出 长边 ≤4096px、总像素 ≤12MP）｜倍率 3×→2×`。

验证：`tests/test_image_upscale.py` 扩到 **10 组**——新增第 10 组「尺寸护栏」：档位解析（默认/非法回落/
大小写/off/自定义，负数与非数字容错）、档位宽严有序、输入拦截（超长边/超像素/恰好达标/尺寸未知）、
自适应降档（不动/降档/降到 1×/连 1× 都不行/off/尺寸未知）、文案，以及「允许倍率回落 → 再按尺寸降档」
两层规则不互相打架。全套回归（workflow_v7 14 组、model_walk、baseworkflow_meta、draw_card、
lora_base_filter、wait_for_result、force_draw、prompt_boost/guard、size_helpers、session_isolation 等）
通过；schema 校验与前端构建通过。
...
现在与出图链路完全对齐（**同一套卡片渲染器、同一套开关**：总开关 / 结果卡开关 / 失败卡开关 /
发送范围 / 自动撤回 / `show_prompt`）：

- **提交成功后 → 处理中卡片**（`state="queued"/"drawing"`）：右上角显示「排队 N」（有队列时，
  取自中转站 `X-Queue-Position`，退回本地队列统计）或倍率；参数胶囊含输入尺寸 / 倍率 / 种子 /
  放大工作流，另外带服务器设备、今日已出图、以及「图片放大：<输入文件名>」；
- **完成 → 报表卡**（输入/输出尺寸、倍率、耗时 + 放大功能与工作流两行），并且**纳入卡片开关**：
  卡片关闭或发送范围不含当前会话时，改为一行文字小结，不会出现「关了卡片却还收到卡片」；
- **失败/超时 → 失败卡**（沿用 `_card_or_text`，文案与原因进卡片）；
- 排队记账与出图共用同一套本地队列（提交时登记、结束（含失败/超时）在 `finally` 里移除），
  `/queuestatus` 看到的排队数把放大任务也算进去。

验证：`test_image_upscale` 8 组、`test_workflow_v7` 14 组等全套回归通过；compileall 通过。

## v7.7.6（「更多功能」页两处 UI 修复：添加走功能选择面板 + 弹窗不再占满全屏）

- **「＋ 添加功能」改为先弹「功能选择面板」**：列出可选的功能类型（目前只有「图片放大（超分）」，
  带图标与一句话说明），点选后再进入配置弹窗 —— 这样以后新增功能类型时只需往这个列表里加一项。
- **修复配置弹窗占满整屏**：插件里 `n-modal` 用的是 `preset="card"`，且经 teleport 渲染到
  `<body>`，**组件内的 scoped 样式命中不到它**，所以此前写在视图里的宽度根本没生效。现按仓库
  既有约定，把宽度统一放进 `App.vue` 全局样式：`.feat-modal{width:640px;max-width:92vw}`、
  功能选择面板 `.feat-modal.pick{width:560px}`，移动端一律 92vw；并给弹窗内表单加
  `max-height:62vh` 内部滚动，表单长时不会把整屏撑到顶。
- 顺带：库里只有一个放大工作流时，新增条目会**自动预选**它，少点一次。

验证：前端 `vite build` 通过，产物里确认 `.feat-modal{width:640px;max-width:92vw}`、
`.feat-modal.pick{width:560px}`、`.feat-modal .n-form{max-height:62vh}` 与移动端 92vw 规则
均已注入。

## v7.7.5（「更多功能」改成可添加多条：每条绑定放大工作流 + 独立配置 + 启用/禁用）

**需求**：像「工作流」页那样管理功能——可以添加多条「图片放大」，每条绑定自己的放大基础工作流，
各自配置（倍率/种子/超时），并可单独启用/禁用。

**配置结构**（新顶层键 `features`，`template_list`；旧的单条 `image_upscale` 保留为兼容读取）：

```
features: [
  { __template_key, kind: "upscale", name: "vosr2", enabled: true, base_id: "3",
    default_scale: 3, allowed_scales: "2,3,4", seed_mode: "random", seed_value: 6666, timeout: 300 },
  ...
]
```

- `kind` 为功能类型判别字段（目前只有 `upscale`；以后加新功能类型时在同一个列表里扩展）；
- `base_id` 绑定一个**放大类**基础工作流（必须是解析通过、`roles.kind="upscale"` 的，绑错会明确报错）；
- **向后兼容**：列表为空而旧版 `image_upscale` 对象里有配置时，运行时把它折算成一条（在「更多功能」
  页保存一次即完成迁移），旧版按「工作流名」绑定的写法也仍能匹配到对应基础工作流。

**指令**：`/图片放大 [功能名] [倍率]`

- 不写名字 → 用**第一个启用**的条目；点名支持：功能名 / 条目 key / 绑定的基础工作流 ID·名字·文件名；
- 点名到**已停用**的条目 → 明确提示「已停用，请到「更多功能」页启用」（而不是含糊的「找不到」）；
- 倍率规则、种子策略、超时全部按**该条目自己的配置**走（不在允许列表内 → 回落该条目的默认倍率）。

**WebUI「更多功能」页**：改成列表式（与「工作流」页同一套交互）——卡片展示名称/绑定工作流/
默认倍率/允许倍率/种子/超时，右上角开关直接启用停用，支持「编辑 / 删除 / ↑↓ 排序」（顺序决定
默认用哪条）；新增/编辑弹窗里可下拉选择放大类基础工作流，并对「未绑定 / 绑的不是放大类 /
解析未通过 / 工作流已被删除」给出不同的警示标签；检测到旧版单条配置时会提示一次并预填。

验证：`tests/test_image_upscale.py` 重写扩到 **8 组**（解析 / 防误判 / 参数 15 组写法 / 倍率规则 /
prompt 注入 / **条目列表与点名（含旧版兼容、停用提示、未配置引导）** / **绑定解析（ID·名字·
唯一·绑错类型·失效·无可用）** / 真实入库链路三处注入）；全套回归通过（workflow_v7 14 组、
model_walk 5 组、baseworkflow_meta 2 组、character 99 项、draw_card、lora_base_filter、
wait_for_result、prompt_boost、prompt_guard、force_draw、session_isolation、nai_params、
size_helpers、image_store_today）；schema 校验与前端构建通过。
...
### 1. 解析器支持「放大类」工作流（新 `kind=upscale`）

- VOSR2 这类工作流**没有采样器、没有提示词**（`LoadImage → 放大节点 → SaveImageExtended`），
  原先被 `parse_workflow` 第一道硬校验直接拒掉（「要求恰好 1 个采样器节点」），根本进不了库；
- 新增旁路解析 `_parse_upscale`：0 采样器时尝试按「纯处理工作流」解析，产出注记
  `kind=upscale`、图输入节点、处理/放大节点、模型加载节点、保存节点、处理链，
  以及**倍率**与**种子**的可写位置；判定很严（必须有图输入 + 保存节点 + **完全没有文本编码器**），
  所以「丢了采样器的出图工作流」不会被误收；连线断开 / 中间没有处理节点 → 针对性报错；
- 倍率识别支持「连线到独立整数节点」：VOSR2 的 `scale` 连的是 `easy int.value`，
  解析器会记到那个节点上（写倍率 = 写它的 `value`）；字面量倍率则就地在放大节点上写。

### 2. 图片放大功能

- 新指令 **`/图片放大 [放大工作流] [倍率]`**（别名：`放大图片` / `图片超分` / `超分`）：
  - **取图**：消息内图片 / 引用消息里的图 / 本插件最近生成的那张（与 `/img2img` 同源兜底）；
  - **工作流**：指令参数 > 配置绑定 > 库里唯一一个放大工作流；支持 ID / 名字 / 文件名包含匹配；
  - **倍率**：`3x` / `x3` / `3倍` / `--倍率 3`（也支持 `--倍数` / `--倍` / `--scale`）/ 纯数字位置参数；
  - **不在允许的倍率里 → 自动回落默认倍率**（防手滑写出 8× 打爆显存）；
- 执行链路与出图**完全独立**：不注入提示词 / 宽高 / LoRA / 采样器，只写「输入图 + 倍率 + 种子」；
  权限总闸（白名单/黑名单）、生图限额、NSFW 护栏、图库归档、配额计数、自动撤回与出图同一套；
- 等待上限用本功能自己的 `timeout`（默认 300 秒，放大比出图慢得多），不套出图那套「单张等待硬上限」；
- 结果发**报表卡**（复用 v7.5.x 的报表渲染器）：输入 / 输出尺寸、倍率、耗时 + 放大工作流；
  卡片画不出来退回一行文字。

### 3. 种子策略（VOSR2 / SeedVR2 属**单步扩散**式复原模型）

种子决定扩散采样的初始噪声：**random**（默认）= 每次随机，同一张图细节会略有差异；
**fixed** = 固定种子，结果完全可复现（便于对比不同倍率 / 参数）。

### 4. WebUI：「功能配置」→「更多功能」页

- 占位页重写为真正的配置页，第一块即**图片放大**：开关 / 默认放大工作流（下拉，只列放大类）/
  默认倍率 / 允许的倍率 / 种子策略与固定值 / 超时；字段文案取自 `_conf_schema.json`，避免两处漂移；
- 库里没有放大类工作流时给出引导（去「基础工作流」页上传）；「基础工作流」页的类型标签新增「放大」；
- 菜单与页面标题改为「更多功能」（PC 侧边栏、移动端抽屉、路由标题同步）；
- 该配置块由「更多功能」页管理，「配置」页不再把它丢进兜底的「其他」分区。

验证：新增 `tests/test_image_upscale.py` 7 组（解析 / 防误判 / 参数 15 组写法 / 倍率规则 /
prompt 注入 / 工作流挑选 / **真实入库链路**）全过；`test_workflow_v7` 13 组等全套回归通过；
compileall、schema 校验、前端 vite build 通过（产物已含新页）。

两点说明：① 图片放大**计入生图限额与图库**（与出图同口径）；② 需先把放大工作流上传到
「基础工作流」库（解析通过后类型显示为「放大」），再到「更多功能」里绑定默认工作流。

## v7.7.0（强制出图：消息前缀标记 `t`，AI 不画就插件补）

**需求**：模型经常「只说不画」（回一句「好，马上画」就结束），而我们的所有闸门都发生在
**工具已经被调用之后**——「一次都没调用」这个方向一直是空白。现在给一个**用户显式声明的强制开关**。

**用法**：消息**行首**加标记（默认 `t`）：
`t发张照片`、`t 一个女孩`、`t 猫耳少女站在雨里`、`t把这张图变成水彩`（带参考图 → 图生图）。
还可以顺带用现有参数语法：`t 真人 一个女孩`（按工作流）、`t nai 1girl`（按平台）、
`t 一个女孩 --lora 薄荷 --w 832 --h 1216`。

**语义是「标记 + 保底」，不是「绕过 LLM 的指令」**（流程仍然是 AI 出图）：

1. **事前**：`comfyui_draw` / `comfyui_img2img` 的说明与 `SKILL.md` 里写明「行首标记 = 必须画出来，
   去掉标记后立即调用工具」→ 支持工具调用的模型**自己就画**（人格化回复、参数选择、多张规划都保留）；
   另外用 `on_llm_request` 给这次请求追加一句「必须调用绘图工具，只回文字算失败」的强指令
   （框架的 `ProviderRequest` **没有 tool_choice**，无法强制选中工具，这一步只能提高概率）。
2. **事后兜底（关键）**：本轮结束（`on_agent_done`）时校验——**这条消息真没画出图** → 插件**自己补画**：
   用去掉标记的消息内容，走完整出图链路（Anima 翻译 / LLM 扩写 / 角色卡锚点 / 外观护栏 / 画质前缀），
   权限与限额和其它入口完全一致（白名单、黑名单、生图限额、NSFW、出图卡片、自动撤回、图库归档）。
   补画前会先说一句（可关），不让图凭空冒出来。

**防误伤**（这是 `t` 这种单字母前缀最大的坑）：
- 必须**行首**，且标记后面**紧跟的不能是英文字母**：`t发张照片` ✅、`t 一个女孩` ✅、
  `thanks` / `the cat is cute` ❌（照常聊天，绝不被强行画图）；标记后没内容 → 不接管；
- **群聊必须 @机器人**（默认开，防路人误触）；私聊不要求（也 @ 不到）；
- 前缀可配置（默认 `t`，可改 `tt` / 画t 等）；整套功能可一键关闭。

**新增配置组「强制出图」**（`force_draw`）：开关 / 前缀 / 群聊需 @ / 事前强指令 /
兜底补画 / 补画前说一句。

验证：新增 `tests/test_force_draw.py` 6 组（配置 / 前缀防误伤 / 群聊 @ 与自定义前缀 /
残留标记剥离 / 标记生命周期（指纹-会话隔离-消费一次-过期）/ **兜底判定**（已出图不补、
没出图才补、开关关闭不补、指纹不符不补））全过；`test_prompt_guard`、`test_prompt_boost`、
`test_session_isolation`、`test_character`(99 项)、`test_workflow_v7`(13 组)、`test_draw_card`、
`test_lora_base_filter`、`test_wait_for_result`、`test_size_helpers`、`test_nai_params`、
`test_image_store_today`、`test_llm_tool_docstrings` 全量回归通过；schema 校验 / compileall / 打包校验通过。

> 顺带修掉一个自己写出来的 bug：标记的内存存储原先用 `self._force_draw_pending`，
> 与同名方法撞车（实例属性会把方法覆盖成 dict，之后调用直接炸）→ 存储改名 `_force_draw_marks`。
> 这条是被新测试当场抓出来的。
...
打出来的包缺这个模块（装上去会 import 失败）。**请直接安装 v7.6.1，不要用 v7.6.0 的包。**

**修复**：

- 清单补上 `prompt_guard.py`；
- 打包脚本加**防漏校验**：仓库根下所有 `*.py`（排除下划线开头的临时脚本）必须都在清单里，
  否则直接报错、删掉半成品 zip 并退出——同类事故不会再静默发生。

验证：重新打包并核对 zip 内条目（`prompt_guard.py` 在包内）。

## v7.6.0（提示词丰富化 + 角色保真代码化）

一次把两件看似矛盾的事同时做掉：**让 AI 写的提示词更丰富**，同时**绝不把角色画错**。
思路是把「维度」切开——**外观维度冻结**（只能来自角色卡锚点/用户明说），**非外观维度放开**。

- **新增 `prompt_guard.py`（护栏模块）**：把「禁止补外观」从一句劝告变成可执行代码
  - 外观维度词表：发色 / 瞳色 / 发型 / 兽化 / 体型 / 服装（中英双语，`white hair` 与 `白发` 都认）；
  - 角色/作品 tag 判定：`name \(work\)` 形态 + 角色卡名单命中 → 外观冻结；
  - 冲突剥除：角色卡锚点是权威，同维度不同值的标签剥掉（多人时括号分组整组保护，不误删别的角色）；
  - 扩写护栏：只增不改（必须保留原文全部标签），新增/改写的外观标签一律剥掉；
  - 画质前缀分档：标签系 / pony / NAI / 自然语言系（后者不加）。
- **画质前缀改由插件确定性追加**（`prompt_boost.quality_prefix`，默认开）：按底模风格自动补，
  已有不重复加。此前这条规则只写在 `skills/` 文档里，而 docstring 又让模型「无需读取任何技能文件」
  → 原生链路基本从来没加过质量词（只有第三方插件改写路径硬塞过）。
- **短描述自动扩写**（`prompt_boost.enhance_llm`，默认开）：满足「标签数 ≤ 阈值 + 不含角色/作品 tag +
  标签系底模 + 请求预算充足」时触发一次 LLM 扩写，**只允许补镜头/构图/光影/氛围/场景/材质**；
  产出经护栏校验（丢原文标签就整条放弃、新增外观被剥除），任何失败回退原文。
- **外观冲突剥除接线**（`prompt_boost.guard_appearance`，默认开）：命中角色卡后自动剥掉与锚点冲突的
  外观标签——历史上「角色本是绿发却被补了 white_hair」那类事故从此由代码兜住；
  用户明确要求改外观（「把头发染成粉色」「换成红裙子」「眼睛改成红色」）时自动放行。
- **docstring 新增「★★★细节密度」一节**：7 项维度清单（主体/外观/表情动作/镜头构图/场景环境/光影氛围/材质细节）、
  标签系 15~25 个 Danbooru 标签下限、自然语言系 1~3 句、并强调**这张清单只覆盖非外观维度**；
  同时写明「画质词插件自动加、你不用写」。img2img 的 docstring 同步。
- **补齐两处缺口**：
  1. **平台链路**（NAI / OpenAI 兼容 / 自定义）此前完全跳过画质前缀与角色卡锚点注入（角色保真为零）
     → 现已补上：NAI 走 nai 档前缀，锚点注入 + 外观护栏与 ComfyUI 链路同一套规则；
  2. **多角色剥分组保护**：某个角色拿不到锚点标签时，原先会无条件剥掉模型自己写的分组
     （锚点与模型分组两边全空 = 该角色特征直接丢失）→ 改为保留模型分组、仅追加锚点标签。
- **SKILL.md 收敛（单真源）**：「画质前缀」一节改为「插件已自动加，你不用写」；
  新增「细节密度」一节；角色章节补「不补外观 ≠ 写得干瘪（镜头光影照写）」与「插件已强制校验」。
  配置页新增「提示词丰富化」分区（`GROUP_META`）。

验证：新增 `tests/test_prompt_guard.py`（7 组）与 `tests/test_prompt_boost.py`（4 组）全过；
`test_character` 99 项、`test_workflow_v7` 13 组、`test_draw_card` 7 组、`test_lora_base_filter` 4 组、
`test_wait_for_result` 6 组、`test_size_helpers` 3 组、`test_nai_params`、`test_llm_tool_docstrings`
全部回归通过；compileall / schema 校验 / 前端构建通过。

## v7.5.4（修复「排队等图再也不提示超时」）

**现象**：排队/后端一忙，等图等再久也不再收到「绘图超时」提示（连失败卡都没有）。

**两个根因**（都在等待链路，叠加起来就是"永远不提示"）：

1. **已等时长算错了**：`ComfyUIClient.wait_for_result` 用 `elapsed += interval` 估算，
   只数 sleep，**每次 `/history` 请求本身的耗时完全不计**。后端一忙（每次请求几百毫秒
   至数秒）或连中转站，真实等待 = 睡眠 + N×请求耗时，能远超设定值，看起来就是"不超时"。
   现改为按 `time.monotonic()` 的真实时间判超时，并且**单次轮询请求单独限时**
   （interval×3，最少 5s / 最多 30s）——此前用的是 client 全局超时 150s，
   一个挂死的响应就能把整轮等到框架超时之后。轮询连续失败现在也会打日志
   （第 1/5/20/50… 次），不再静默空转。
2. **等待超时没算"已经花掉的时间"**：`draw_wait_hard_cap` 只掐等待时长，而 LLM 改写/
   翻译、参考图上传、提交这些前置耗时不在其内。LLM 翻译 30s + 等待 100s = 130s >
   AstrBot「工具调用超时」120s → 提示还没发出去，协程就被框架硬取消（静默）。
   现改为**整次请求预算**：等待上限 = min(动态值, 硬上限 − 已耗时 − 8s)，
   预留 8 秒专门用于发送超时提示；预算被前置耗时吃光时也至少等 5 秒，
   保证「有结果 / 没结果」都能给出明确反馈。配置 hint 与日志都写明这层含义
   （日志新增「本次已耗时 Xs，等待超时按预算收紧为 Ys」）。

验证：新增 `tests/test_wait_for_result.py` 6 组（慢后端按真实时间超时 / 单次请求限时 /
命中即返回 / 后端持续报错仍按时超时 / 已有历史不空等 / 预算收紧含预留与下限）全过；
`test_workflow_v7` 13 组、`test_draw_card` 7 组、`test_lora_base_filter` 4 组回归通过；
compileall 与 schema 校验通过。

## v7.5.3（新增/编辑工作流：LoRA 下拉按底模筛选）

- **WebUI 新版工作流编辑器：LoRA 下拉改为按该工作流的底模筛选**（此前把全库 LoRA 都列出来）。
  根因：过滤读的是旧字段 `editForm.base_model`，而新版工作流走 `base_id` 关联基础工作流、
  该字段恒为空 → 过滤条件恒真。现改为「基础工作流的底模名 → 旧版字段」依次取值；
- 底模匹配规则（与后端同一套）：任一侧为空（通用 / 不限）即算可用；否则小写原文相等，
  再去标点后相等 / 前缀 / 包含 —— 「Z-Image Turbo」↔「z-image-turbo」、「Krea 2」↔「krea2」、
  「Anima Pencil XL v5」↔「anima」都能对上，避免写法差异导致漏配；
- LoRA 页新增「只显示匹配底模的 LoRA」开关（默认开）与筛选状态提示
  （`已按底模「X」筛选：6 / 42 个可用`），让「少了一些 LoRA」可见可解释；
  该底模下确实没有匹配项时自动放行全部，不会出现空下拉；关闭筛选时列表里标注每个 LoRA 的底模；
- **顺带修同类 bug**：聊天指令 `/绘图lora` 也是只读 `wf["base_model"]` 判适用性 →
  新版工作流下等于不过滤。现统一走 `_wf_base_model`（新版取基础工作流底模）+ 同一套匹配规则，
  并修掉卡片详情「可用 LoRA」的同款取值。

验证：新增 `tests/test_lora_base_filter.py`（归一化 / 12 组匹配用例 / 底模解析 / 适用性判定）全过；
`test_workflow_v7.py` 13 组、`test_draw_card.py` 7 组回归通过；compileall 与前端构建通过。

## v7.5.2（/绘图排行 也改为报表卡）

- **/绘图排行** 接入报表卡：两块大数字（上榜人数 / 合计出图）+ 前 5 名列表
  （名次 + 名称在左、出图张数右对齐，榜首带强调色圆点高亮），右上显示统计范围；
- 过滤口径不变（真人出图，已排除伴侣插件自动生图），footer 写明口径；
- 同样**卡片优先、文字兜底**：渲染/发送失败自动退回原来的文字排行榜；
- 无人生图时也发卡（「还没有人生图～」），不裸文字。

验证：test_draw_card 第 7 组扩展（排行卡），7 组全过；compileall 通过；样张人工核对。

## v7.5.1（/绘图统计、/绘图状态 改为报表卡）

- `draw_card.py` 新增通用**报表卡**渲染器（`render_report` / `save_report`）：
  与出图卡同源——同一套 9 套主题、随包字体回退链、footer（说明+时间 / 右下品牌）；
  版式：右上范围胶囊 + 大数字块（tiles）+ 分区两列行（左名称右数值，支持绿点/红点状态）；
- **/绘图统计**：三块大数字（累计出图 / 范围出图 / Token 用量）+ 热门工作流列表
  （张数 · 平均速度），右上显示统计范围；
- **/绘图状态**：服务器行（绿点=正常含延迟与队列状态、红点=不可达与原因，仍不暴露名称/IP）
  + 生图限额分区（开关 / 总·时·日 / 管理员豁免 / 今日全群已生图）；
- 两个指令都是**卡片优先、文字兜底**：报表卡渲染/发送失败自动退回原来的文字版
  （文字构建逻辑原样保留），不会没有输出；
- 报表卡走统一「图片发送+自动撤回」入口，同样不会在群里越堆越多。

验证：`tests/test_draw_card.py` 新增第 7 组（统计卡/状态卡/全缺省/9 套主题/落盘）7 组全过；
compileall 与前端构建通过；样张人工核对（浅色统计卡 + 深色状态卡，`design_lab/out/`）。
...
掉进了兜底的「其他」分区——和 schema 里的超时配置隔着老远。已加入「出图行为」分组，
排在「每队列任务额外超时」与「出图超时上限」之间（schema 里的顺序本来就是对的，
只是控制台配置页是前端写死的分组清单——`permissions`/`recall` 在 v5.12.9 也栽过一次）。

验证：前端构建通过。

## v7.4.10（排队等待超时被硬掐 100 秒 → 硬上限可配置）

**现象**：出图超时配了 300 秒、额外超时 0、上限 0，但排在队列后面的任务 150 多秒就报
「等待 100 秒仍无结果」。

**原因**：等待循环里写死了 `timeout = min(动态值, 100)`——这个 100 是为了小于 AstrBot
框架的「工具调用超时」（默认 120 秒）：LLM 对话出图走框架的工具调用，等待超过框架值会被
**硬取消工具协程**，打断 bot 的 WS 连接。初衷没错，但写死且无日志，用户配的 300 秒被静默
砍掉，完全无从排查（失败卡上只写「等待 100 秒」）。

**修复**：

- 硬上限改为配置项 **「单张等待硬上限」`draw_wait_hard_cap`**（默认 100 保持兼容，
  0 = 不限制）：最终等待 = min(出图超时 + 队列累加, 出图超时上限, 硬上限)；
- 配置 hint 与 `comfyui_draw` 工具 docstring 都写明配套关系：**LLM 对话出图**必须先把
  AstrBot 的「工具调用超时」调大（插件建议 300 秒），再把本值设为略小于框架值（如 290）；
  纯指令 / 伴侣插件场景可以设 0 不限；
- 等待超时的计算过程现在**打日志**：
  `【队列】 本次等待超时 290s（基础 300s + 排队 1×300s，动态上限 9300s，硬上限 290s）`，
  以后再被掐一眼就知道是被哪一层掐的。

**给你的建议配置**：出图超时 300 → AstrBot「工具调用超时」设 300+ → 本插件「单张等待
硬上限」设 **290**（或 0，若你接受 LLM 出图可能被框架取消的风险）。

验证：`test_workflow_v7`（13 组）等全过；compileall 与前端构建通过；schema 校验通过。
...
- 绘制中 / 失败卡片的「尺寸」不再显示「(默认)」——四层尺寸决策（用户传参 > 比例词 >
  工作流默认 > 兜底 512）必定解析出具体数字，那才是真实会用的尺寸，直接展示；
- 极端情况下（如图生图）拿不到注入值时，用**实际输入尺寸**（参考图 / 工作流 JSON 的
  latent 值，v7.4.4 引入的 `_in_w/_in_h`）兜底；再拿不到才显示「(默认)」（理论不可达）。

验证：`test_workflow_v7`（12 组）全过；compileall 通过。

## v7.4.6（卡片右上角状态改为胶囊徽章，「排队 0」也显示）

- 「排队 N」不再分有无排队：**一直显示**（0 = 空闲），配合 v7.4.3 的「入队后发卡 +
  不再补发文字提示」，卡片上的队列信息就是唯一信息源，不会打架；
- 状态行从裸文字升级为**胶囊徽章**：淡强调底 + 描边，标签用副色、**数字用强调色且稍大**
  （排队 N / 耗时 X 秒通用）——右下角 / 失败原因板同款视觉语言；
- 修一个渲染坑：胶囊与失败原因板的**半透明底**此前直接画在不透明底图上，Pillow 的
  `ImageDraw` 不做 alpha 混合（v7.2.0 踩过的同一个坑），深色客户端下会变成一坨深色；
  改为「独立图层 + alpha_composite」，深浅主题观感一致。

验证：`tests/test_draw_card.py` 5 组全过；compileall 与前端构建通过；
深浅两套主题样张人工核对（`design_lab/out/real_task_teal.png` / `real_task_night.png`）。

## v7.4.5（结果卡瘦身：只报结果 + 修提示词折行只占半宽）

两条反馈：

- **提示词只占左边一半**：`_wrap()` 折行时拿「超采样后的字体度量」直接和「输出像素宽度」比，
  单位差了一倍——实际折行宽度只有一半。已在函数内部把宽度 ×S 换算，提示词 / 失败原因都恢复
  **占满整卡宽度**（长提示词卡从 660px 降回 576px，行数少了近一半）。测试第 5 组加了反向断言：
  最长一行的实际宽度必须接近折行宽度（>60%），这类单位 bug 以后跑测试就能抓到。
- **结果卡不该重复绘制中卡片的内容**：结果卡定位就是替代原来那句
  「出图完成后发送一条包含尺寸/大小/耗时/时间的小报告」——所以现在**只报结果**：
  尺寸 / 大小 / 耗时（时间在页脚），不再带 LoRA、采样参数、提示词（平台链路同理）。
  各司其职：绘制中卡看参数、结果卡看结果、失败卡看原因。
- 配置项「出图完成发结果卡」的说明同步改写。

验证：`tests/test_draw_card.py` 5 组全过（含新加的「折行必须用满宽度」断言）；
`test_workflow_v7`（12 组）、`test_image_store_today`、`test_nai_params`、
`test_llm_tool_docstrings` 全过；compileall 与前端构建通过。

## v7.4.4（图库「输入尺寸 / 放大」适配旧版工作流）

**问题**：大图信息的「输入尺寸」只有新工作流才有，旧版工作流没有。

**原因（不是做不到，是我当时漏了）**：v7.1.0 加这两栏时写的是「本次注入的 w×h」。
新工作流尺寸由基础工作流解析而来、**一定会注入**，所以有值；旧版工作流在几种情况下**不注入宽高**
（`resolution_mode=none`、找不到 latent 节点、图生图尺寸由参考图决定），此时注入值≠真实输入尺寸，
写入无效值等于没有 → 面板按「无数据不显示」处理。

**修复**：

- 新增 `_latent_size_of()`：没注入宽高时，直接读**已加载的工作流 JSON** 里 latent 的
  `width/height`（`EmptyLatentImage` 及其别名、自定义类名兜底、字符串数值都能读），
  这才是真正的输入尺寸；
- 新增 `_images_size_of()`：图生图用**参考图尺寸**（取第一张能读到的）；
- 归档改写「实际输入尺寸」`_in_w/_in_h`（注入值 / JSON 原值 / 参考图尺寸，三选一），
  日志打印取值来源；三项都拿不到就留空（不写假值）；
- 顺带把「**放大**」栏也适配：旧版工作流没有解析注记、`_upscale_note()` 返回空时，
  用「出图尺寸 ÷ 输入尺寸」推算（新增 `_upscale_measured_note()`），文案如实写
  「约 2.00×（按出图/输入尺寸推算）」，**不谎称知道模型名**；尺寸没变大（±2% 内）不显示。

**注意**：本次之前已出图的历史记录仍没有这两列数据（v7.1.0 之前的老图更是如此）——
面板对无数据的记录照旧不显示假值；要看到旧版工作流的输入尺寸，重新出一张即可。

验证：新增 `tests/test_size_helpers.py`（用 ast 从 `main.py` 摘出三个纯函数单独执行：
latent 标准/别名/自定义类名、字符串数值、读不到留空；参考图顺延读取；倍率推算的
2× / 无放大 / ±2% 容差 / 缺值）全过；`test_workflow_v7`（12 组）、`test_draw_card`、
`test_image_store_today`、`test_nai_params`、`test_llm_tool_docstrings` 全过；
compileall 与前端构建通过。

## v7.4.3（出图卡片改为「入队成功后」发送）

原来卡片是**提交前**发的，有两个副作用：右上角「排队 N」只能拿本地队列快照（与中转站实际位置可能不一致，
出现过「卡片写 0、下一条文字说 1」）；提交失败时会先发一张「绘制中」再补失败卡。改为**入队成功后**发：

- 卡片右上角「排队 N」改用真实队列位置（中转站 `X-Queue-Position` 响应头优先，无则本地队列），
  与排队的现实一致；页脚时间也正好是提交时刻（那句「参数以提交时刻为准」名副其实）；
- 无排队时仍不写「排队 0」；
- **提交失败不再先发卡片**：只发一张朱红失败卡（提交异常 / 未返回任务号两种都走失败卡）；
- 文字排队提示仍在，但**只在卡片没发出去时**才发（开关关闭、私聊、渲染失败等），避免重复；
- 平台链路（NAI / OpenAI 兼容 / 自定义）不变：平台是「一次阻塞请求拿图」，没有入队阶段，
  卡片仍发在请求之前、结果卡发在拿到图之后（代码里已注明原因）；
- 配置项说明同步为「入队后（排队中 / 绘制中）」。

验证：`test_draw_card`（5 组）、`test_workflow_v7`（12 组）、`test_image_store_today`、
`test_nai_params`、`test_llm_tool_docstrings` 全过；compileall 与前端构建通过。

## v7.4.2（出图卡片：提示词不再省略）

- 卡片底部提示词改为**完整显示**：按宽度折满所有行，不再截断成两行加省略号
  （超长提示词的卡片会相应变高，卡片高度本来就随内容自适应）；
- 折行函数 `_wrap()` 的 `max_lines=0` 表示不限行；只有「失败原因」这种兜底场景才限行
  （8 行，极端超长才省略号），并让失败原因板**随行数变高**，多行报错能完整看到；
- 配置项 `show_prompt` 的说明同步改为「完整显示、自动折行」。

验证：`tests/test_draw_card.py` 第 5 组改为「提示词完整显示（不省略）+ 失败原因多行」
（断言折行结果不含省略号且拼回原文一字不少、长提示词卡片比短的高、多行失败原因卡更高）全过；
`test_workflow_v7`（12 组）、`test_image_store_today`、`test_nai_params`、
`test_llm_tool_docstrings` 全过；compileall 与前端构建通过。

## v7.4.1（出图卡片：修「结果卡开了不发」+ 今日出图数口径 + 去掉多余排队文案）

四条反馈逐条修：

- **左上不再显示「底模 / 模型」四个字**：kicker 直接显示底模名（ComfyUI）或模型名（平台）；
- **今日出图数口径改为「当天全部用户」**：此前用的是自管计数器（只在本次升级后才开始累计，
  数字偏小）；现在改为查图库，口径与「绘图统计」一致——`source='gen'` 且 `status=0` 且未删除、
  按本地日期；图库不可用时才退回自管计数（新增 `ImageStore.count_today_generated()`）；
- **修「结果卡开了也不发」**：门槛误留成旧开关 `show_draw_report`（默认关），现在改为卡片配置的
  「出图完成发结果卡」`result_enabled`（默认开）。同时补齐日志：
  · `_send_draw_card` 每个「不发」分支都打日志（总开关关 / 结果卡关 / 失败卡关 / 发送范围仅群聊而当前是私聊 /
    渲染失败并提示常见原因）；
  · 结果卡流程加 else 分支，明确记录「为什么没发」（被 NSFW 拦截 / 报告已并入配文 / 开关或范围原因）；
- **去掉提交后那条多余排队文案**：提交前已发卡片时不再发「前面还有 N 个，轮到就给你。⏳」
  ——既重复、又和卡片上的数字打架（卡片是提交前快照、那条提示来自中转站的入队响应）。
  同时「无排队」时卡片右上角不再写「排队 0」，宁可不显示（有排队才写，避免与中转站实际队列对不上）。

验证：新增 `tests/test_image_store_today.py`（全部用户 / 参考图不计 / 内容去重 / 跨天不计）
全过；`test_workflow_v7`（12 组）、`test_draw_card`、`test_nai_params`、`test_llm_tool_docstrings`
全过；compileall 与前端构建通过。

## v7.4.0（旧版工作流一键转新版）

- 「旧版工作流」页每张卡片新增 **转新版** 按钮，页面右上新增 **批量转新版**；
- **转换规则**：用旧条目的「工作流文件名」匹配**基础工作流**（原始文件名 `file_name` → 去扩展名 →
  显示名 `name`，大小写不敏感）；**匹配到才允许转换**——新版工作流必须有 `base_id` 才能解析节点。
  没填文件名 / 没匹配到 / 基础工作流解析未通过，都会**弹窗给出具体原因**（并提示怎么补），
  不会静默失败；
- **转换动作**：写入 `base_id`，条目自然从「旧版工作流」页消失、出现在「工作流」页（前端转换后刷新列表）；
- **封面**：原封面原样保留；原本没有封面时，自动把基础工作流的封面**复制**一份到 `lora_assets/`
  再挂上（复制而不是引用 `bm:` 前缀——大图详情走 `lora/image`，只认 lora_assets）；
- **顺带修「默认封面」**：此前只把基础工作流的封面名写进 `wf.image`（而工作流封面读的是
  `lora_assets/`），点了看着没反应；新增 `workflows/use_base_cover` 由后端复制封面文件后再写入；
- **独立 WebUI 通道**补上 `/workflows/` 前缀分发（此前 standalone 通道下 `workflows/sampler`
  一直是 404），并登记本次新增的两个接口；
- 顺手修：`tests/test_workflow_v7.py` 里存在两段 `if __name__ == "__main__"`（后者覆盖前者），
  实际只跑了前 5 组；已合并为一段。

验证：`test_workflow_v7` 从「只跑 5 组」恢复为 **12 组全过**（新增第 12 组：文件名匹配的
精确 / 去扩展名 / 大小写不敏感 / 显示名兜底 / 未命中返回 None）；`test_draw_card`、
`test_nai_params`、`test_llm_tool_docstrings` 全过；compileall 与前端构建通过。

## v7.3.0（出图卡片重做：9 套主题 + 排队/绘制/完成/失败四态 + 平台自适应）

按讨论定稿落地：底模 / 状态 / 工作流在左上，队列·设备·今日出图数在右上，时间挪到 footer，
右下品牌右对齐；绘制失败也要发卡；出图结果信息由文字改为卡片；工作流名前的圆点居中。

- **新增 `draw_card.py`**（独立渲染模块，纯函数便于单测）：
  - **9 套主题**：teal 青碧（默认）/ night 夜幕（深色）/ graphite 石墨灰 / indigo 靛蓝 /
    violet 紫罗兰 / amber 琥珀橙 / sunset 落日橙 / rose 樱粉 / crimson 朱红；
    **绘制失败与被拦截自动切朱红**，不受配置主题影响；
  - **四态**：排队中 / 绘制中 / 已完成 / 绘制失败（大标题与右上首行随状态变化：排队时「排队 N」、
    完成/失败时「耗时 X 秒」）；
  - **版式**：左上「底模 → 状态大字 → ● 工作流（或平台）」；右上「状态行 / 设备 / 今日已出图 N 张」；
    中部「失败原因板 / LoRA / 参数 / 提示词（最多两行）」；footer「参数以提交时刻为准 · 完整日期时间」
    + 右下品牌（渐变小方块「萌」+ 名字，右对齐且与文字垂直居中）；
  - 工作流名前的小圆点按**文字墨迹中心**对齐（此前是固定像素，看着偏上）；
  - **平台自适应**：第三方平台出图时底模位显示模型名、工作流位显示平台名、右上设备位显示「云端 XX」、
    **LoRA 区整块隐藏**、参数换成平台参数（模型/尺寸/步数/引导/采样器/噪声调度/画师串/种子）；
  - **字体随包**：`assets/fonts/ResourceHanRoundedCN-Medium.woff2`（已在打包清单内），
    缺失时回退插件 `fonts/` 与系统字体，并打日志；
- **今日出图统计**：`data_dir/draw_stats.json` 自管计数，只统计成功发出的图（被拦截的不算），
  跨天自动归零，失败数单独记；卡片右上角展示；
- **main.py 接线（三个时机全部发卡）**：
  - 提交前卡：排队中 / 绘制中，含 LoRA（名称 + 生效权重）与参数胶囊；
  - **结果卡**：出图完成后发结果卡（尺寸/大小/耗时/步数/CFG/采样器/调度器/噪点/放大/种子 + 提示词），
    替代原来的文字小报告；开启结果卡时不再把报告并进图片配文（避免同一信息出现两次），
    卡片画不出来才退回文字小报告；
  - **失败卡**：工作流加载失败、参考图上传失败、提交失败、未返回任务号、等待超时、无图/节点报错、
    配置错误、平台生图失败/无图 —— 全部走失败卡（原因即原来那条文字），渲染失败自动退回文字，
    保证用户一定有反馈；
  - 第三方平台链路：提交前与出图完成也发卡（与 ComfyUI 同构）；
  - 卡片发送走统一的「图片发送 + 自动撤回」入口，卡片不会在群里越堆越多；
  - 删除 v7.2.0 的旧双列渲染器（`_render_draw_card` / `_card_font_path` / `_hex_rgb`）与
    `title` / `gradient_from` / `gradient_to` 配置项（标题改为状态，配色改为主题）；
- **配置项**（`_conf_schema.json` 的 `pre_draw_card`）：启用 / 主题 / 发送范围 / 显示提示词 /
  今日出图数 / 结果卡 / 失败卡 / 品牌名 / 自定义字体（留空即用随包字体）。

验证：新增 `tests/test_draw_card.py`（9 主题等高、四状态、失败强制朱红、平台卡更矮、渲染落盘、
今日统计自增与跨天归零、超长提示词/原因截断）全过；`test_workflow_v7`（11 组）、`test_nai_params`、
`test_llm_tool_docstrings` 全过；compileall 与前端构建通过。

## v7.2.0（新增：生图前「出图信息卡片」，渐变底 + 霞鹜文楷）

新配置分组 **「🎴 出图信息卡片」**（`pre_draw_card`，**默认开启**）：

- **发送时机**：提交给 ComfyUI **之前**（此时参数已全部定稿）；默认**仅群聊**，
  可切「所有会话」。渲染/发送失败只记日志，绝不阻断出图；
- **卡片内容**（有则显示、没有就跳过）：服务器（名字｜设备配置）、队列
  （提交那一刻前面还有几个任务）、工作流（+文生图/图生图）、参考图张数（图生图）、
  LoRA（名称:权重）、尺寸（本次请求 w×h）、放大（解析放大模型名 → `4×（4x-UltraSharp.pth）`）、
  步数 / CFG / 采样器 / 调度器 / 噪点 / 种子；可选显示提示词（截断 160 字）；
- **渲染**：本地 Pillow 画 PNG（不依赖 AstrBot 的 t2i 服务）——对角渐变底 + 圆角 +
  半透明信息面板 + 双列排版，2x 超采样抗锯齿；文字与面板颜色按渐变亮度**自适应**
  （深底白字/深面板，浅底深字/白面板）；
- **字体**：默认 `fonts/LXGWWenKai-Regular.ttf`（正文）+ `LXGWWenKai-Medium.ttf`
  （标题/标签），可在配置里改文件名或填绝对路径；**缺失自动回退系统字体**。
  霞鹜文楷全量版单个约 24MB，**不入 zip、不入库**（已放好并写进 .gitignore），
  换机器时把 ttf 放进插件目录 `fonts/` 即可；
- 卡片渲染产物落在 `data_dir/card_render/`，已纳入既有的定期清理（按 keep_temp_hours）。

**踩到的 Pillow 坑（记录备查）**：`ImageDraw.Draw(im, "RGBA")` **不做 alpha 混合**，
是把半透明像素直接写进画面——深色面板会变成「半透明黑」，在浅色底/客户端白底上显示成
浅灰，配白字直接看不见（第一版实测就是白字消失）。现改为「独立图层 + `alpha_composite`」
真正混合，文字一律用不透明色。

验证：从 `main.py` 摘出渲染相关方法原样执行，用真实字体渲染 3 张卡片并肉眼核对
（深色渐变 780×598 / 含提示词长行 / 浅色渐变深字分支），排版与配色正常；
compileall、schema 校验、前端构建通过。

## v7.1.0（服务器唯一 key 绑定 + 设备配置 + 图库输入尺寸/放大倍率）

**① 服务器：唯一 key 绑定（改名不再断链）**

- 新增条目自动生成唯一 key（`__template_key`），**只读展示、前后端都保证唯一**：
  - WebUI 配置页把 `__template_key` 从可编辑输入框改为只读行（显示 key + 「只读：工作流
    绑定服务器用的就是它，改名不影响」）；新增条目前端直接生成唯一 key
    （此前写死 `"default"`，多条目撞成同一个 key）；
  - 后端初始化补齐逻辑升级：不只补空值，**重复 key 也重新生成**；WebUI 保存兜底同样
    改为生成唯一值（此前也写死 `"default"`）；
- 工作流绑定字段新增 **`server_key`**：下拉 **label = 服务器名字（+设备+（默认））**、
  **value = 唯一 key**；卡片/详情显示的服务器名由 key 反查（旧数据标「旧版绑定」）；
  打开旧工作流时自动把「名字」迁移成 key，重新保存即完成升级；
- 后端 `_resolve_server()`：先按 key 匹配，匹配不到再按**名字**兼容匹配（打日志提示）；
- **默认服务器语义明确**：`enabled` 的说明改为「设为默认服务器」——启用的那台就是默认；
  多台启用时取列表第一台并打警告（v7.1.0 起**不再报错**，此前直接失败）；
- 服务器新增 **`device`**（字符串，如 `RTX 4090D 24G｜i9-13900K｜64G`），仅展示用；
- 本地队列的服务器标识改用唯一 key（改名后队列统计不再断成两份）。

**② 图库：输入尺寸 + 放大倍率**

- `images` 表新增三列 **`in_w` / `in_h` / `upscale`**（建表 DDL、迁移元组、INSERT、UPDATE、
  行转字典五处同步），出图归档时写入：
  - `in_w/in_h` = **请求尺寸**（注入工作流的 w×h；平台链路记为平台请求尺寸）；
  - `upscale` = 放大倍率说明，新增 `_upscale_factor_of_model()` 从放大模型名解析
    （`4x-UltraSharp.pth`→4×、`RealESRGAN_x2plus.pth`→2×），`_upscale_note()` 优先读
    改写后的工作流节点 `model_name`、其次配置值，**`upscale_mode=bypass` 或无放大返回空**，
    **解析不到倍率只给模型名**（不瞎猜）；
- 图库大图详情新增「输入尺寸」「放大」两行；**老图这两列无数据 → 不显示**（不假装有）。

验证：从 `main.py` 源码摘出改动代码原样执行，4 组用例全过（服务器 key/旧名字/不存在/
默认启用/多启用取第一/都没启用；放大倍率 4x·x2·2x·ESRGAN_4x·自定义名→None·空；
展示文案；放大说明的节点优先/配置兜底/bypass 空/无放大空）；compileall、schema 校验、
前端构建通过。

字体（霞鹜文楷）已取回并放到插件根 `fonts/`（Regular + Medium，各约 24MB）：
**不入库、不进 zip**（体积原因），运行时装信息卡片会用；缺失时回退系统字体。
卡片功能在 v7.2.0。

## v7.0.19（修「编辑预设」跳转体验：深链自动展开分组）

反馈：「管理预设」按钮点了只是跳到配置页，还得自己翻找，看不懂它有什么用。

- 按钮改名 **「编辑预设（配置页）↗」**，并改为**深链**：跳转时带
  `?group=尺寸比例预设`，配置页接收后**自动展开该分组并滚动定位**到它
  （ConfigView 新增 `applyDeepLink()`，schema 加载完与 query 变化时都会触发）；
- 「档位 × 比例对照表」弹窗底部也加了同一个「编辑预设」按钮（点了自动关弹窗再跳），
  并在说明里写清：对照表在弹窗里看、要改基准尺寸去配置页那个分组。
- 定位失败（分组因配置项被清空而不存在）时不折腾，静默返回。

验证：前端构建通过。

## v7.0.18（尺寸体系重做：1K 基准矩阵 + 档位缩放 + 工作流档位上限 + 对照表）

**口径（与用户确认）**：档位按**总像素预算**定义，不是长边——
1K≈1.0MP（SDXL 原生）、1.5K≈2.3MP、2K≈4.2MP（默认上限）、4K≈8.3MP（＝UHD 像素量）。
倍率是线性边长倍率：1K×1、1.5K×1.5、2K×2、4K×2.9。
这样「4K」不会变成 4096²＝16.8MP（方形）那种必爆显存的尺寸。

- **比例预设换成 1K 基准矩阵**（`draw_ratio` 默认值 + 一次性迁移）：改用 SDXL 原生训练
  尺寸 —— 方形 1024²、竖版 2:3 **832×1216**、横版 3:2 1216×832、7:9 / 9:7、4:7 / 7:4、
  9:16 640×1536、16:9 1536×640；关键词只放**比例词**（档位由倍率负责）。老用户若预设
  仍是旧内置内容会自动升级（带 `_size_presets_v2` 标记）；自己改过则原样保留并打日志。
- **档位机制（后端内置）**：`_SIZE_TIERS`（倍率）+ `_SIZE_TIER_KEYWORDS`
  （1k/1K/标清、1.5k、2k/2K/高清、4k/4K/超清，英文整词匹配，`4kings`/`12k` 不误命中）；
  `_scale_size_to_tier()` 等比缩放、`_cap_size_to_tier()` 按像素预算降级、
  全部对齐 16 的倍数（降级后按 16 递减保证不超预算）。
- **尺寸决策四层（顺序固定）**：
  ① 输入侧：用户显式传参 > 用户原话比例词/档位词 > 工作流默认宽高；
  ② **禁止改变尺寸（lock_size）**：非图生图时恒用工作流默认宽高，忽略①；
  ③ **最高支持档位**（新增 `max_size_tier`，默认 2K）：超预算等比降级，不放大；
  ④ **最大宽/高**（max_width/max_height）：最后硬裁剪。
  【宽高】日志完整输出每一层来源，排查不用再猜。
- **配置/UI**：工作流新增「最高支持尺寸档位」下拉（尺寸页）；尺寸页新增
  「档位 × 比例对照表」弹窗（比例 × 1K/1.5K/2K/4K 的实际像素，数据来自后端
  `_size_tier_table()`，新路由 `/size_tiers`）；配置页「尺寸比例预设」分组 hint 重写。
- **LLM 侧**：工具说明明确「用户说档位词（1K/2K/4K/标清/高清/超清）时不要传 width/height」，
  交给插件按档位缩放（自己填像素会绕过档位逻辑与上限）。

验证：从 `main.py` 源码摘出档位机制整块原样执行，5 组用例全过——
档位词识别（含 `4kings`/`12k` 不误命中）、832×1216 在四档的缩放（1K/1.5K/2K/4K）、
上限裁剪（2K 预算 4.19MP、1K 降级、4K 请求降为 1696×2464 且比例一致、非法档位按默认）、
16 对齐、对照表（方形 1K→4K：1024²/1536²/2048²/2976²；竖版 2:3：832×1216→2416×3520）；
compileall 与前端构建通过。

## v7.0.17（修复尺寸被比例预设悄悄改写 + 尺寸预设接进工作流表单）

**问题一：说「用默认尺寸」却没按工作流配置的尺寸**

根因：`_ratio_src = positive or ""`——尺寸比例（draw_ratio）的关键词匹配扫的是
**LLM 生成的画面描述**，而内置 keyword 里含 `portrait` / `square` / `wide` /
`landscape` / `vertical` 这些英文词。自然语言系底模（Qwen/FLUX 等）写一句
"portrait of a girl" 就会命中「竖版 3:4」，把工作流默认尺寸 816×1216 换成 864×1152，
用户完全看不出是谁改的（日志只写「比例预设（关键词命中，覆盖工作流默认）」）。

修复（`main.py`）：

- 新增 `_ratio_source_text()`：比例匹配文本改为**用户自己说的话**（`event.message_str`），
  取不到才退回画面描述；
- 新增 `_RATIO_SKIP_RE`：用户说「默认尺寸 / 按工作流默认 / 用默认」时**完全跳过**比例预设；
- 英文关键词改为**整词**匹配（`portrait` 不再命中 `portraiture`、`wide` 不命中 `widespread`）；
- 【宽高】诊断日志补充「比例匹配文本来源 + 命中文本片段」，下次一眼看出是谁改的尺寸。

**问题二：老的比例预设「找不到、没被用起来」**

- 预设一直都在（配置页 → 现在是独立分组）：本版把 `draw_ratio` 从「默认工作流」里
  拆出来，新增独立配置分组 **「尺寸比例预设」**（📐），不再藏在默认工作流里；
- 工作流编辑弹窗「尺寸」页新增 **「尺寸预设（来自配置页）」下拉**：选中就把该档位的
  宽高填进默认宽高（可再手改），右侧「管理预设 ↗」直接跳配置页；
  内置档位：方形 1:1（1024²）、竖版 3:4（864×1152）、竖版 2:3、竖版 9:16、
  横版 4:3 / 3:2 / 16:9、长图 1:2 等。

验证：从 `main.py` 源码摘出改动函数原样执行，7 个用例全过——用户原话命中「竖版」正常；
「用默认尺寸 / 按工作流默认」不触发；`portrait`/`wide` 整词命中而 `portraiture`/
`widespread` 不命中；显式传参优先；停用档位不参与；匹配文本来源用户原话优先；
复现案（用户说「画一张写真」+ LLM 写 "portrait of a girl"）不再改尺寸。
`main.py` compileall 与前端构建通过。

## v7.0.16（修复：底模的「提示词风格 / 优先语种」没传给 LLM）

现象：底模库里配了「提示词风格=自然语言、优先语种=中文」，LLM 出词还是英文 Danbooru
标签写法——等于配置没生效。

根因：这两个字段只在**出图时**被读（`_load_from_base` 里换算成 `is_anima` 决定要不要
翻译），**从未进入 LLM 上下文**。LLM 看到的工具说明里，「提示词规范」是按模型家族名
（anima/illustrious/pony/qwen…）硬编码的一份清单，跟底模库配置无关，所以它只会按名字猜。

修复：

- 新增 `_basemodel_of_workflow()`：新版按 `base_id → 基础工作流 → basemodel_id` 取底模，
  旧版按 `base_model` 名称/文件名在底模库匹配（大小写无关；取数异常返回 None 不抛）；
- `comfyui_workflows` 列表每条工作流新增两行：
  `· 提示词风格：自然语言（可掺杂标签）/ Danbooru 标签｜优先语种：中文/英文（底模 X）`
  与 `· 底模提示词规范：<底模 description>`；列表顶部加一句总纲（写 prompt 前先看风格/语种）；
- `comfyui_draw` 工具说明的「提示词规范」章节加最高优先级规则：**先按列表标注的
  风格/语种**（自然语言→按优先语种写整句、不写标签与质量词；Danbooru→英文标签），
  未标注时才回落到原家族规则；
- 工具 docstring 同步说明新标记含义。

验证：从 `main.py` 源码摘出该函数原样执行，7 个用例全过——新版 base_id 取到底模、
未关联底模返回 None、旧版按文件名/名称（大小写无关）匹配、匹配不到返回 None、
取数异常不抛、列表文案确实带出「风格｜语种」；`main.py` compileall 通过。

已知边界：本版只是让 LLM **知道**该用哪种风格/语言；插件不会把 LLM 写出的中文整句
自动翻译成英文（只有标签系工作流会做中文→标签翻译），所以 LLM 遵守与否决定最终效果。
