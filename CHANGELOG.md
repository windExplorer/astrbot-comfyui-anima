...
## v5.6.7（摘要日志：槽位文字去掉「键=值」字段观感，明示为自然语言）

- `main.py` 绘图摘要日志的「槽位文字」改用 `键「值」`(中文书名号) 打印，去掉 `bubble=...; bottom=...`
  的「键=值」字段观感，避免与「槽位2 该用自然语言而非字段」的约定混淆。
  说明：槽位2(boogu) 实际发给 ComfyUI 的提示词一直是纯自然语言
  (`_render_nl_slot` 把「一致性锁+自然语言中段+风格锁」拼成一段中文)，日志里的 `bubble/bottom`
  仅是槽位名(键)，并非写进提示词的字段语法。
