...
## v5.11.11（性能：大幅缩短出图前的「前戏」等待）

针对「每次绘图前戏要搞半天」，逐段优化出图前的准备耗时：

- **翻译改为并发 + 缓存（最大头）**：原先 `_translate_prompt` 按逗号**逐段串行**翻译——提示词里有 N 个中文片段就是 N 次串行网络往返（danbooru / LLM / HTTP），这是前戏耗时的主因之一。现改为：①多个中文片段**并发**翻译（并发上限 4，避免打爆翻译服务）；②结果进内存缓存 `(mode, 原文片段)`，同一片段下次直接命中、零网络开销。日志会打印 `含中文片段=N 个（缓存命中=x，待翻译=y）`。
- **LLM 改写/翻译改为「提前发起、并行等待」**：原先在参考图上传**之后**才串行执行。现在提前 `create_task`，与工作流 JSON 加载、参考图上传并行；上传失败会主动取消该任务，不白跑。
- **参考图并行上传**：图生图多张参考图原先逐张串行上传（网络往返叠加），现并发上传，总耗时≈最慢一张。
- **temp 清理 / 图库 LRU 节流**：原先每次出图都同步扫 temp 目录 + 跑图库 LRU（阻塞事件循环，大图库下明显拖慢准备阶段）。现默认至少间隔 600s 才真正执行一次。
- **已读回执非阻塞**：贴表情是一次平台网络往返，原先 `await` 在出图关键路径上；现丢后台任务执行。
- **新增耗时日志**：提示词定稿处打印「出图前 LLM 整理耗时」，提交前打印「前序准备合计」——可据此定位慢在哪一段（LLM 整理 / 上传 / 工作流准备）。
