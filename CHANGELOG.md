...
- **`draw_auto` 拆出「默认出图张数」配置 `default_count`**：v4.3.3 里「默认几张」与「单次上限」合并在一个 `max`，无法区分。现拆成两个独立配置：
  - `default_count`（默认 3）：AI 对话中用户说「来几张 / 来多张」或完全没提数量时，默认出这么多张；
  - `max`（默认 3）：单次请求最多出图数（硬上限），无论默认张数还是用户明确说的具体数都不会超过它（管理员豁免时可突破）。
- **`comfyui_draw` / `comfyui_img2img` 的 `count` 参数改为 0 表示「未指定」**：模型不传 `count`（用户没说数量或说「来几张」）时走 `default_count` 出默认张数；明确传了具体数（如「来 3 张」）才按具体数出。工具描述与参数说明已同步，避免「来几张」被当成默认 1 张。
