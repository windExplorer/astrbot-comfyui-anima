...
- **修复图生图兜底把旧图混进参考图导致结果污染**：当 LLM 已通过 `image` 参数（或事件内）明确提供本次参考图时，不再回退混入 `g_last_generated`（本插件历史生成图）。此前在 Agent 走 `comfyui_img2img` 且 `image` 参数下载成功的情况下，兜底逻辑仍会把上几次生成的 `.webp` 一并注入，造成「注入 3 张参考图」、重绘被旧图干扰。`image` 参数已成功取到图时仅静默跳过 `g_last_generated` 兜底，避免污染。
