...
- **修复「LLM 默认把任务识别成图生图、文生图被误判」**：此前 `img2img_workflow` 参数（LLM/伴侣文生图也会顺带带上默认图生图工作流）被当作图生图意图的一部分，导致 `is_img2img` 常为 True；再叠加历史/会话/生成图兜底捞图，文生图就真被跑成图生图了。现**明确**：`is_img2img` 只由「显式传了 `image` 参数（无论解析成败）」或「本次消息/引用里真有图」决定；只带 `img2img_workflow` 而无图一律按文生图处理。文生图但带了 `img2img_workflow` 时，仅用于按风格回退到对应文生图工作流（真人图生图→真人文生图，动漫图生图→动漫文生图），避免跑错风格，但绝不做图生图、也不从中途兜底捞旧图。
