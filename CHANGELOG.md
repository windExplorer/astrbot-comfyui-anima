...
- **新增「工作流别名」配置 `workflow_aliases`**：把外部调用方（如伴侣插件）传入的、本插件不存在的工作流名映射到真实工作流。格式为多条「别名=真实工作流名」，用逗号或换行分隔，如 `anime_selfie_workflow=动漫`、`ComfyUI default=动漫`。`_resolve_workflow` 匹配前会先按别名解析（大小写不敏感、忽略首尾空白）；未命中别名的未知工作流仍按原逻辑回退默认工作流。WebUI 配置页对该字段渲染多行 textarea，支持换行输入。
